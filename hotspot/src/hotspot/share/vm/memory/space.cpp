...
HeapWord* CompactibleSpace::forward(oop q, size_t size,
  ...
  while (size > compaction_max_size) {	//空闲空间不够存储该active对象
    // switch to next compaction space
    cp->space->set_compaction_top(compact_top);
    cp->space = cp->space->next_compaction_space();	//取下一个内存区来存储该active对象

    if (cp->space == NULL) { //取下一个内存代中的内存区来存储该active对象
...
/**
 * 填充内存碎片(为了减少内存区压缩时大量的对象移动,允许存在一定的内存碎片)
 */

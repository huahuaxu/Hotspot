...
void GenCollectedHeap::post_initialize() {
  ...
  TwoGenerationCollectorPolicy *policy = (TwoGenerationCollectorPolicy *)collector_policy();
 ...
 * 执行一次GC
 *
 * @param full 执行Full Gc还是Minor Gc(增量式Gc)
 * @param clear_all_soft_refs 本次Gc是否需要清理所有的软引用(也由内存堆分配回收策略决定)
 * @param size 本次Gc之后待分配的内存块大小
 * @param is_tlab 本次Gc之后是否从线程的本地缓冲区中分配内存块
 * @param max_level 本次Gc中允许回收的最老内存代
 */
void GenCollectedHeap::do_collection(bool  full,
  ...
  {
    FlagSetting fl(_is_gc_active, true);	//当前线程正式开始GC
    ...
    increment_total_collections(complete);	//更新Gc计数器
    ...
    if (full) {
      ...
      //如果是当前进行的是Full GC,则从最老的内存代开始向前搜索，找到第一个可收集所有年青代的内存代
    ...
    for (int i = starting_level; i <= max_level; i++) {
      if (_gens[i]->should_collect(full, size, is_tlab)) {	//是否回收当前的内存代
    	//如果当前回收的内存代最老代，则本次GC升级为Full Gc
        ...
        //统计当前内存代本次Gc的时间消耗信息,Gc次数,内存变化信息
        ...
        //正式开始回收当前的内存代
        {
          ...
          printf("%s[%d] [tid: %lu]: 开始回收内存代[%d: %s]的内存垃圾(full=%s, do_clear_all_soft_refs=%s)...\n", __FILE__, __LINE__, pthread_self(),
    ...
    //打印出本次Gc之后,内存堆的变化情况,如果本次是一次Full Gc,则打印出永久代的内存变化情况
    if (PrintGCDetails) {
    ...
    if (complete) {
      ...
      printf("%s[%d] [tid: %lu]: 试图调整永久代[%s]的大小(只有在一次Full Gc之后才会调整永久代大小).\n", __FILE__, __LINE__, pthread_self(), perm()->as_gen()->name());
 ...
 * 		2).Java方法栈上的对象
 * 		3).本地方法栈上的对象
 * 		4).比待回收的内存代Older的内存代上分配的所有对象
 * 		5).[比待回收的内存代Younger的内存代上分配的所有对象]
 *
 * @param level		当前将要进行Gc的内存代
 * @param younger_gens_as_roots 是否把比当前内存代年青的内存代上分配的对象看作根对象
 * @param collecting_perm_gen 是否回收永久代上的垃圾对象
 * @param
 * @param
 */
void GenCollectedHeap::gen_process_strong_roots(int level,
  ...
  printf("%s[%d] [tid: %lu]: 开始搜索当前JVM上所有的根对象...\n", __FILE__, __LINE__, pthread_self());

  if (!do_code_roots) {
    SharedHeap::process_strong_roots(activate_scope, collecting_perm_gen, so, not_older_gens, NULL, older_gens);
  } else {
    bool do_code_marking = (activate_scope || nmethod::oops_do_marking_is_active());
    CodeBlobToOopClosure code_roots(not_older_gens, /*do_marking=*/ do_code_marking);
    SharedHeap::process_strong_roots(activate_scope, collecting_perm_gen, so, not_older_gens, &code_roots, older_gens);
  }

  //把比内存代level年青的内存代上分配的对象都看作根对象
  if (younger_gens_as_roots) {
    if (!_gen_process_strong_tasks->is_task_claimed(GCH_PS_younger_gens)) {
      for (int i = 0; i < level; i++) {
    	printf("%s[%d] [tid: %lu]: 开始遍历内存代[%s]上的对象,并将其作为根对象...\n", __FILE__, __LINE__, pthread_self(), _gens[i]->name());
...
/**
 * 执行Full Gc,用于外部命令: "jmap -histo:live"/"jmap -dump:live*"
 */
...
bool GenCollectedHeap::create_cms_collector() {
  ...
  printf("%s[%d] [tid: %lu]: 试图创建一个CMS垃圾回收器...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * 根据对象的物理地址计算其所在的内存区(某一个内存代中的内存区)
 */
...
bool GenCollectedHeap::block_is_obj(const HeapWord* addr) const {
  ...
  if (perm_gen()->is_in_reserved(addr)) {	//对象在永久代
...
/**
 * 内存堆可分配给线程本地分配缓冲区的最大空间
 */
size_t GenCollectedHeap::unsafe_max_tlab_alloc(Thread* thr) const {
  size_t result = 0;
  for (int i = 0; i < _n_gens; i += 1) {
    if (_gens[i]->supports_tlab_allocation()) {	//内存代支持给线程被地分配缓冲区分配内存
...
/**
 * 内存堆中各内存代的压缩预处理: 计算每个内存代中活着的对象在其内存区压缩后的偏移位置
 */
void GenCollectedHeap::prepare_for_compaction() {

  Generation* scanning_gen = _gens[_n_gens-1];	//最后一个内存代
...
//记录Gc之前各内存代的堆顶位置(该位置可能是其其下次分配内存的开始位置)
...
/**
 * 处理指定对象转存储到指定内存代失败的情况
 *    1.尝试依次升级对象到之后的每一个内存代
 *    2.尝试以扩展内存的方式依次升级对象到指定内存代及之后的每一个内存代
 */

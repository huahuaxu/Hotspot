...
class CollectedHeap : public CHeapObj {
 ...
 protected:
  ...
  unsigned int _total_collections;          // (Minor+Full)gc总次数
  unsigned int _total_full_collections;     // Full Gc总次数
...
class GCCauseSetter : StackObj {
 ...
 public:
  GCCauseSetter(CollectedHeap* heap, GCCause::Cause cause) {
    ...
    _heap = heap;
    _previous_cause = _heap->gc_cause();	//上一次触发GC的原因
    _heap->set_gc_cause(cause);				//通知内存堆管理器本次触发Gc的原因
  }

...
template <class T> void objArrayKlass::do_copy(arrayOop s, T* src, arrayOop d, T* dst, int length, TRAPS) {
  ...
  if (s == d) {	//源数组和目的数组是同一个数组，则应该考虑相见的部分丢失问题
    ...
    klassOop bound = objArrayKlass::cast(d->klass())->element_klass();	//目的数组元素的类型描述
    klassOop stype = objArrayKlass::cast(s->klass())->element_klass();  //源数组元素的类型描述
    if (stype == bound || Klass::cast(stype)->is_subtype_of(bound)) {
       //目的数组的元素类型与源数组的类型匹配或者是其父类
      ...
      for (T* p = dst; from < end; from++, p++) {
        ...
        if (element_is_null || Klass::cast((new_val->klass()))->is_subtype_of(bound)) {
...
void objArrayKlass::copy_array(arrayOop s, int src_pos, arrayOop d, int dst_pos, int length, TRAPS) {
  ...
  if (UseCompressedOops) {
    narrowOop* const src = objArrayOop(s)->obj_at_addr<narrowOop>(src_pos);//源数组的开始复制位置
    narrowOop* const dst = objArrayOop(d)->obj_at_addr<narrowOop>(dst_pos);//目的数组的开始赋值位置
    do_copy<narrowOop>(s, src, d, dst, length, CHECK);
  } else {
    oop* const src = objArrayOop(s)->obj_at_addr<oop>(src_pos);//源数组的开始复制位置
    oop* const dst = objArrayOop(d)->obj_at_addr<oop>(dst_pos);//目的数组的开始赋值位置

...
class CMSCollector: public CHeapObj {
 ...
 protected:
  ...
  MemRegion                      _span;    //旧生代与永久代两个连续内存区
  CardTableRS*                   _ct;      // card table

  // CMS marking support structures
  CMSBitMap     _markBitMap;			//标记的根对象存储位图
...
class CMSExpansionCause : public AllStatic  {
 public:
  enum Cause {
    _no_expansion,
    _satisfy_free_ratio,	//满足空闲率的配置
    _satisfy_promotion,		//存储由年青代升级过来的对象
    _satisfy_allocation,	//分配内存时容量不足
    _allocate_par_lab,
...
class ConcurrentMarkSweepGeneration: public CardGeneration {
 ...
 protected:
  ...
  CompactibleFreeListSpace*  _cmsSpace;  //当前内存代的核心内存区
  ...
  NOT_PRODUCT(
    size_t _numObjectsPromoted;		//升级对象的数量
    size_t _numWordsPromoted;		//升级对象的总大小(占内存)
 ...
 private:
  ...
  double _initiating_occupancy;	//触发当前内存代进行CMS垃圾回收的内存使用率
 ...
 public:
  ...
  /**
   * 来自年青代的通知:其内存分配达到了内存限制,裁决其新的分配限制位置
   */
  HeapWord* allocation_limit_reached(Space* space, HeapWord* top,
                                     size_t word_size);
...
class MarkFromRootsClosure: public BitMapClosure {
  CMSCollector*  _collector;
  MemRegion      _span;				//标记的内存区
  CMSBitMap*     _bitMap;			//标记位图
  CMSBitMap*     _mut;
  CMSMarkStack*  _markStack;		//被标记过的对象存储栈(用于递归迭代标记其引用对象)

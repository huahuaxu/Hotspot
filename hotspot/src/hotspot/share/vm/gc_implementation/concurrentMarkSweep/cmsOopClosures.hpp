...
class MarkRefsIntoClosure: public OopsInGenClosure {
 private:
  const MemRegion _span;	//待标记对象所在的内存区
...
class MarkRefsIntoAndScanClosure: public OopsInGenClosure {
 private:
  MemRegion          _span;					//待标记对象所在的内存区
  CMSBitMap*         _bit_map;				//对象标记位图
  CMSMarkStack*      _mark_stack;			//标记对象存储栈(等待标记该对象的所有引用对象)
...
class PushOrMarkClosure: public KlassRememberingOopClosure {
 private:
  ...
  CMSBitMap*      _bitMap;		//对象标记位图
  CMSMarkStack*   _markStack;	//标记对象存储栈(等待标记该对象的所有引用对象)
  HeapWord* const _finger;		//对象标记位图当前的扫描位置

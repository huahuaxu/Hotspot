...
PSYoungGen*  ParallelScavengeHeap::_young_gen = NULL;	//年青代内存管理器
...
jint ParallelScavengeHeap::initialize() {
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化当前的内存堆管理器...\n", __FILE__, __LINE__, pthread_self());
  ...
  const size_t og_page_sz = os::page_size_for_region(yg_min_size + og_min_size,
                                                     ...
                                                     8);
  const size_t pg_page_sz = MIN2(os::page_size_for_region(pg_min_size, pg_max_size, 16),
                                 og_page_sz);
  ...
  printf("%s[%d] [tid: %lu]: 开始为内存堆申请内存(yg_max_size=%lu, og_max_size=%lu, pg_max_size=%lu)...\n",
		  __FILE__, __LINE__, pthread_self(), yg_max_size, og_max_size, pg_max_size);
  ...
  if (UseCompressedOops) {	//对象压缩存储
    if (addr != NULL && !heap_rs.is_reserved()) {	//从指定的内存地址开始申请内存堆的空间失败

      // Failed to reserve at specified address - the requested memory
      // region is taken already, for example, by 'java' launcher.
      // Try again to reserver heap higher.
      addr = Universe::preferred_heap_base(total_reserved, Universe::ZeroBasedNarrowOop);
      ReservedHeapSpace heap_rs0(pg_max_size, pg_align, og_max_size + yg_max_size, og_align, addr);

      if (addr != NULL && !heap_rs0.is_reserved()) {
        // Failed to reserve at specified address again - give up.
        addr = Universe::preferred_heap_base(total_reserved, Universe::HeapBasedNarrowOop);
        assert(addr == NULL, "");
        ReservedHeapSpace heap_rs1(pg_max_size, pg_align, og_max_size + yg_max_size, og_align, addr);
        heap_rs = heap_rs1;
      } else {
        heap_rs = heap_rs0;
      }
  ...
  if (!heap_rs.is_reserved()) {//内存堆的空间申请失败
  ...
  if (_barrier_set == NULL) {
    vm_shutdown_during_initialization("Could not reserve enough space for barrier set");
    return JNI_ENOMEM;
  }
  ...
  ReservedSpace perm_rs = heap_rs.first_part(pg_max_size);	//永久代内存空间
  ReservedSpace main_rs = heap_rs.last_part(pg_max_size, og_align);	//新生代+旧生代内存空间
  ...
  printf("%s[%d] [tid: %lu]: 试图创建内存代管理器[AdjoiningGenerations]...\n", __FILE__, __LINE__, pthread_self());
  _gens = new AdjoiningGenerations(main_rs,
                                   og_cur_size, og_min_size, og_max_size,
  ...
  printf("%s[%d] [tid: %lu]: 试图创建永久代[Perm_Gen]: 初始大小(bytes)=%lu, 最小大小(bytes)=%lu, 最大大小(bytes)=%lu ...\n", __FILE__, __LINE__, pthread_self(), pg_cur_size, pg_cur_size, pg_max_size);
  ...
  printf("%s[%d] [tid: %lu]: 试图创建GC工作线程管理器(GCTaskManager)...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * 从堆中分配指定大小的一块内存空间(核心功能)
 *  分配策略:
 * 		1).试图从年青代中申请存储空间;
 * 		2).分配成功则直接返回，否则到3);
 * 		3).加全局锁锁,再次试图从年青代中申请存储空间;
 * 		4).分配成功则直接返回，否则到5);
 * 		5).试图从老年代中申请存储空间，成功则返回;
 * 		6).如果其它线程触发了Gc,则等待Gc的完成,之后回到3);
 * 		7).触发一个GC操作
 */
  ...
  *gc_overhead_limit_was_exceeded = false;
  ...
  while (result == NULL) {
    ...
    // total_collections() value!
    {
      ...
      printf("%s[%d] [tid: %lu]: 试图从老年代中申请 %lu bytes存储空间...\n", __FILE__, __LINE__, pthread_self(), size * HeapWordSize);
      ...
      if (GC_locker::is_active_and_needs_gc()) {	//其它线程已经触发了Gc操作
        ...
        if (!jthr->in_critical()) {	//等待Gc被完成
          MutexUnlocker mul(Heap_lock);	//释放堆上的锁
...
/**
 * 从老年代中申请指定大小的一块内存存储空间
 */
HeapWord* ParallelScavengeHeap::mem_allocate_old_gen(size_t size) {
	//如果无法在年青代中分配或者Gc已经被触发,则在年老代中分配
...
HeapWord* ParallelScavengeHeap::permanent_mem_allocate(size_t size) {
  ...
  do {
    ...
    // total_collections() value!
    {
      ...
      printf("%s[%d] [tid: %lu]: 试图从永久代[Perm_Gen]中申请 %lu bytes存储空间...\n", __FILE__, __LINE__, pthread_self(), size * HeapWordSize);
    ...
    if (result == NULL) {
      ...
      if (limit_exceeded) {
        size_policy()->set_gc_overhead_limit_exceeded(false);
        if (PrintGCDetails && Verbose) {
          gclog_or_tty->print_cr("ParallelScavengeHeap::permanent_mem_allocate: return NULL because gc_overhead_limit_exceeded is set");
        }

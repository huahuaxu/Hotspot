...
bool Verifier::verify(instanceKlassHandle klass, Verifier::Mode mode, bool should_verify_class, TRAPS) {
  ...
  if (is_eligible_for_verification(klass, should_verify_class)) {
    ...
    if (UseSplitVerifier && klass->major_version() >= STACKMAP_ATTRIBUTE_MAJOR_VERSION) {
    ...
    if (TraceClassInitialization) {
      if (HAS_PENDING_EXCEPTION) {
        ...
        tty->print_cr(" exception pending %s ",instanceKlass::cast(PENDING_EXCEPTION->klass())->external_name());
...
Symbol* Verifier::inference_verify( instanceKlassHandle klass, char* message, size_t message_len, TRAPS) {
...
void ClassVerifier::verify_method(methodHandle m, TRAPS) {
  ...
  u4 code_length = m->code_size();//方法的字节码长度

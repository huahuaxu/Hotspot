...
jint dump_heap(AttachOperation* op, outputStream* out) {
  ...
  if (path == NULL || path[0] == '\0') {
	printf("%s[%d] [tid: %lu]: 外部命令[dumpheap]没有指出输出路径!!!\n", __FILE__, __LINE__, pthread_self());
    out->print_cr("No dump file specified");
  } else {
    bool live_objects_only = true;   //仅仅dump出active对象
    const char* arg1 = op->arg(1);
    if (arg1 != NULL && (strlen(arg1) > 0)) {
      if (strcmp(arg1, "-all") != 0 && strcmp(arg1, "-live") != 0) {
    	printf("%s[%d] [tid: %lu]: 外部命令[dumpheap]的参数只能是 -all||-live, 而当前的参数是%s!!!\n", __FILE__, __LINE__, pthread_self(), arg1);
...
static AttachOperationFunctionInfo funcs[] = {
  ...
  { "dumpheap",         dump_heap },	//dump内存堆的相关信息: "jmap -dump:*"
#endif  // SERVICES_KERNEL
  { "load",             JvmtiExport::load_agent_library },
  { "properties",       get_system_properties },
  { "threaddump",       thread_dump },
  { "inspectheap",      heap_inspection },	//dump内存堆中对象统计信息 : "jmap -histo:*"
  { "setflag",          set_flag },
...
/**
 * "Attach Listener"线程的处理逻辑
 *  接受外部命令并在执行后返回
 */
static void attach_listener_thread_entry(JavaThread* thread, TRAPS) {

  printf("%s[%d] [tid: %lu]: 线程[Attach Listener]开始正式工作(核心功能)...\n", __FILE__, __LINE__, pthread_self());
  ...
  for (;;) {
    ...
    printf("%s[%d] [tid: %lu]: 获取到一个外部命令[%s].\n", __FILE__, __LINE__, pthread_self(), op->name());
...
/**
 * 创建"Attach Listener"线程来接收外部命令并执行返回结果
 */
void AttachListener::init() {
  EXCEPTION_MARK;

  printf("%s[%d] [tid: %lu]: 开始创建[Attach Listener]线程...\n", __FILE__, __LINE__, pthread_self());

  //创建一个java.lang.Thread实例对象
  ...
  //调用java.lang.Thread的构造方法Thread(ThreadGroup, String)
  ...
  //调用java.lang.ThreadGroup的方法add(Thread)

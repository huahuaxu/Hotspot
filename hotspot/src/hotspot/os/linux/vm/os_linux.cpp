...

  {
    /* sysclasspath, java_home, dll_dir */
    {
        ...
        *(strrchr(buf, '/')) = '\0';  /* get rid of /libjvm.so */
        ...
        strcpy(dll_path, buf);
        printf("%s[%d] [tid: %lu]: 设置JVM所依赖的(Sun实现)动态链接库搜索路径[%s]..\n", __FILE__, __LINE__, pthread_self(), dll_path);
        ...
        printf("%s[%d] [tid: %lu]: 设置JVM的根路径[%s]..\n", __FILE__, __LINE__, pthread_self(), home_path);
     ...
     */
    {
        ...
        printf("%s[%d] [tid: %lu]: 设置用户java程序所依赖的第三方动态链接库搜索路径[%s]..\n", __FILE__, __LINE__, pthread_self(), ld_library_path);
     ...
     */
    {
        ...
        printf("%s[%d] [tid: %lu]: 设置扩展类加载器(ExtClassLoader)搜索的类路径[%s]..\n", __FILE__, __LINE__, pthread_self(), buf);
...
/**
 * 获取操作系统的当前时间(1970-01-01到现在的时间距离，单位ms)
 *
 * struct timeval{
 * 		long int tv_sec; // 秒数
...
/**
 * 获取操作系统的当前时间(精确到纳秒)
 * 如果系统不支持纳秒级别的计时，返回的还是us*1000
 *
 * 原型：		int clock_gettime(clockid_t clk_id, struct timespect *tp);
 * 参数说明:	clockid_t clk_id 用于指定计时时钟的类型，有以下4种:
...
void * os::dll_load(const char *filename, char *ebuf, int ebuflen)
{
  ...
  printf("%s[%d] [tid: %lu]: 试图打开动态链接库(RTLD_LAZY)[%s]...\n", __FILE__, __LINE__, pthread_self(), filename);
...
UserHandler(int sig, void *siginfo, void *context) {
  ...
  printf("%s[%d] [tid: %lu]: 接受到一个信号[%d]，并将其通知给[Signal Dispatcher]线程，让其分发处理.\n", __FILE__, __LINE__, pthread_self(), sig);
...
void* os::signal(int signal_number, void* handler) {
  ...
  printf("%s[%d] [tid: %lu]: 重置信号[%d]的处理函数..\n", __FILE__, __LINE__, pthread_self(), signal_number);
...
// function returns NULL to indicate ailure.
/**
 *
 * 从操作系统申请指定大小的一块内存空间(内存映射文件的方式实现)
 *
...
/**
 *	向操作系统申请指定大小的内存，并映射到用户指定的内存空间中(共享内存的方式实现)
 *
 ...
 *          	ENOMEM：核心内存不足
 *
 * 参数说明: 	key,    0(IPC_PRIVATE)：会建立新共享内存对象
 ...
 * 		    		PC_CREAT|IPC_EXCL：如果内核中不存在键值与key相等的共享内存，则新建一个共享内存；如果存在这样的共享内存则报错
 *
 * ******************************************************************************************************************
 *
...
char* os::reserve_memory_special(size_t bytes, char* req_addr, bool exec) {
  ...
  printf("%s[%d] [tid: %lu]: 开始创建一块大小为%lu bytes的共享内存...\n", __FILE__, __LINE__, pthread_self(), bytes);
  ...
  if (shmid == -1) {
     ...
     printf("%s[%d] [tid: %lu]: 创建一块大小为%lu bytes的共享内存失败!\n", __FILE__, __LINE__, pthread_self(), bytes);
     return NULL;
  }
  ...
  //如果共享内存连接失败,则直接删除该共享内存，如果共享内存连接成功,则只有当进程结束或调用shmdt()时才删除该共享内存
...
/**
 * 从指定的内存空间位置开始分配一块指定大小的内存(内存映射文件的方式)
 */
char* os::attempt_reserve_memory_at(size_t bytes, char* requested_addr) {
  ...
  //内存空间申请成功,但不是指定的起始位置,则释放该内存映射空间
  if (addr != NULL) {
...
int os::sleep(Thread* thread, jlong millis, bool interruptible) {
  ...
  if (interruptible) {	//允许该线程在休眠中可被其它线程中断
    jlong prevtime = javaTimeNanos();//当前的系统时间

    for (;;) {
      if (os::is_interrupted(thread, true)) {	//检测当前线程是否被其它线程中断休眠
        return OS_INTRPT;
      }
...
int os::naked_sleep() {
	printf("%s[%d] [tid: %lu]: os::naked_sleep()...\n", __FILE__, __LINE__, pthread_self());
 ...
 * 当前线程主动让出本次cpu
 *
 * the calling thread relinquish the CPU and thread is moved to the end of the queue
 * for its static priority. then a new thread gets to run.
 *
 */
void os::yield() {
  sched_yield();
...
void os::interrupt(Thread* thread) {
  ...
  if (!osthread->interrupted()) {
    ...
    OrderAccess::fence();
    ParkEvent * const slp = thread->_SleepEvent ;
    if (slp != NULL) slp->unpark() ;	//如果当前线程在休眠，则让其退出休眠状态
  }
  ...
  ParkEvent * ev = thread->_ParkEvent ;	//如果当前线程被暂停，则让其退出休眠状态
...
int os::open(const char *path, int oflag, int mode) {
  ...
  if (strlen(path) > MAX_PATH - 1) {
    errno = ENAMETOOLONG;
    printf("%s[%d] [%lu]: 试图打开/创建文件[%s]失败，因为文件路径名太长!\n", __FILE__, __LINE__, pthread_self(), path);
    return -1;
  }
  ...
  {
    struct stat64 buf64;
    int ret = ::fstat64(fd, &buf64);	//由文件描述词取得文件状态
    int st_mode = buf64.st_mode;

    if (ret != -1) {
      if ((st_mode & S_IFMT) == S_IFDIR) {
        errno = EISDIR;
        printf("%s[%d] [%lu]: 打开/创建文件[%s]失败，因为文件是一个目录!\n", __FILE__, __LINE__, pthread_self(), path);
        ::close(fd);
        return -1;
      }
    } else {
...
/**
 * 根据当前文件的读指针来计算还有多少bytes的数据未读
 *
 * 系统函数： int fstat(int filedes,struct stat *buf)
 * 数据结构： struct stat {
 * 				dev_t st_dev;  设备
 *				ino_t st_ino;  节点
 *				mode_t st_mode; 模式
 ...
 *				}
 * 说明:    通过属性mode可以判断给定的文件是一个普通文件还是一个目录,链接等等
...
void os::pause() {
  ...
  printf("%s[%d] [tid: %lu]: 试图打开文件(O_WRONLY+O_CREAT+O_TRUNC)[%s]...\n", __FILE__, __LINE__, pthread_self(), filename);
  int fd = ::open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0666);
  if (fd != -1) {
    struct stat buf;
    ::close(fd);

    printf("%s[%d] [tid: %lu]: 一直等待直到确保文件[%s]可用...\n", __FILE__, __LINE__, pthread_self(), filename);
...
static struct timespec* compute_abstime(timespec* abstime, jlong millis) {
  ...
  jlong seconds = millis / 1000;	//计算秒部分
  millis %= 1000;
  if (seconds > 50000000) { // see man cond_timedwait(3T)
    seconds = 50000000;
  }
  abstime->tv_sec = now.tv_sec  + seconds;
  long       usec = now.tv_usec + millis * 1000;	//计算微妙部分
  if (usec >= 1000000) {
    abstime->tv_sec += 1;
    usec -= 1000000;
  }
 ...
 * 函数说明：让互斥锁上锁，如果该互斥锁已被另一个线程锁定和拥有，则调用该线程将阻塞，直到该互斥锁变为可用为止。
 * 函数返回：在成功完成之后会返回零。其他任何返回值都表示出现了错误。
 *
 * 函数原型：int pthread_cond_timedwait(pthread_cond_t *cond, pthread_mutex_t *mutex,
 * 									  const struct timespec *abstime);
 * 函数说明：如果在给定时刻前条件没有满足，则返回ETIMEOUT，结束等待。线程挂起释放mutex锁，离开pthread_cond_wait()之前，mutex将被重新加锁
 *
 */
int os::PlatformEvent::park(jlong millis) {
  ...
  int ret = OS_TIMEOUT;
  int status = pthread_mutex_lock(_mutex);//开始给互斥锁上锁
  assert_status(status == 0, status, "mutex_lock");
  guarantee (_nParked == 0, "invariant") ;
  ...
  while (_Event < 0) {
    status = os::Linux::safe_cond_timedwait(_cond, _mutex, &abst);	//开始等待
 ...
 * 函数说明：	释放被阻塞在指定条件变量上的一个线程，它最多只给一个线程发信号。
 * 			假如有多个线程正在阻塞等待着这个条件变量的话，那么是根据各等待线程优先级的高低确定哪个线程接收
 * 			到信号开始继续执行。如果各线程优先级相同，则根据等待时间的长短来确定哪个线程获得信号。
 * 函数返回：	函数成功返回0；任何其他返回值都表示错误。如果没有线程处在阻塞等待状态,也会成功返回。
 */
void os::PlatformEvent::unpark() {
  ...
  if (v < 0) {	//有线程被暂停
     ...
     AnyWaiters = _nParked ;	//暂停的线程数
     assert (AnyWaiters == 0 || AnyWaiters == 1, "invariant") ;

     //当前操作系统支持NPTL(Native POSIX Thread Library)
     //NPTL解决了这个问题：在某些线程的实现中，会造成等待线程从内核中唤醒（由于cond_signal)然后又回到
     //					内核空间（因为cond_wait返回后会有原子加锁的行为），造成的性能问题

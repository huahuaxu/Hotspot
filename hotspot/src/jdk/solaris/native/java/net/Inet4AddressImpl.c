...
/**
 * 通过DNS获取域名对应的所有ip地址(一个域名可能对应多个ip地址)
 */
JNIEXPORT jobjectArray JNICALL Java_java_net_Inet4AddressImpl_lookupAllHostAddr(JNIEnv *env, jobject this,
                                                jstring host) {
    const char *hostname;
    jobjectArray ret = 0;
    ...
    if (hp != NULL) {
        ...
        i = 0;
        while (*addrp) {
        	//创建一个java.net.Inet4Address对象
          jobject iaObj = (*env)->NewObject(env, ni_ia4cls, ni_ia4ctrID);
          if (IS_NULL(iaObj)) {
            ret = NULL;
            goto cleanupAndReturn;
          }
          //将ip地址设置到java.net.Inet4Address对象的属性address(int)上
          (*env)->SetIntField(env, iaObj, ni_iaaddressID, ntohl((*addrp)->s_addr));
          //将ip地址设置到java.net.Inet4Address对象的属性hostName(String)上
          (*env)->SetObjectField(env, iaObj, ni_iahostID, host);
          //将java.net.Inet4Address对象添加到数组java.net.InetAddress的i位置
          (*env)->SetObjectArrayElement(env, ret, i, iaObj);
          addrp++;
          i++;
        }
    } else {
        JNU_ThrowByName(env, JNU_JAVANETPKG "UnknownHostException", (char *)hostname);
        ret = NULL;
    }

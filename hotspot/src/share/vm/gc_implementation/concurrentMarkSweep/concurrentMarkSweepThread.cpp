...
void ConcurrentMarkSweepThread::run() {
  ...
  while (!_should_terminate) {
    ...
    printf("%s[%d] [tid: %lu]: 线程[%s]试图触发一次后台CMS Gc...\n", __FILE__, __LINE__, pthread_self(), name());
...
/**
 * 阻塞直到可以执行下一次的增量式CMS Gc
 * 2中工作模式:
 * 	1.其它线程触发通知[CMSIncrementalMode]
 * 	2.定时轮训检测[CMSWaitDuration]
 */
void ConcurrentMarkSweepThread::sleepBeforeNextCycle() {
  while (!_should_terminate) {
    if (CMSIncrementalMode) {	//开启了增量式CMS Gc模式
      icms_wait();
      return;
    } else {
...
/**
 * 开启增量式CMS Gc(通知后台线程可以不停地对老年代进行CMS Gc)
 */
void ConcurrentMarkSweepThread::start_icms() {
  //确保当前JVM配置的CMS垃圾回收策略并配置了增量式CMS Gc模式
...
void ConcurrentMarkSweepThread::icms_wait() {
  ...
  if (_should_stop && icms_is_enabled()) {
    ...
    printf("%s[%d] [tid: %lu]: 当前后台线程开始等待,直到一次增量式CMS Gc被触发...\n", __FILE__, __LINE__, pthread_self());

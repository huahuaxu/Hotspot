...
{
  HeapWord* bottom = (HeapWord*) _virtual_space.low();	//内存空间的开始位置
  HeapWord* end    = (HeapWord*) _virtual_space.high();	//内存空间的开结束位置
  ...
  if (_cmsSpace == NULL) {
    vm_exit_during_initialization("CompactibleFreeListSpace allocation failure");
  }
  ...
  if (CollectedHeap::use_parallel_gc_threads()) {	//Gc时多线程工作模式
    typedef CMSParGCThreadState* CMSParGCThreadStatePtr;

    _par_gc_thread_states = NEW_C_HEAP_ARRAY(CMSParGCThreadStatePtr, ParallelGCThreads);
...
void ConcurrentMarkSweepGeneration::init_initiating_occupancy(intx io, intx tr) {
  ...
  printf("%s[%d] [tid: %lu]: 当前内存代[%s]触发CMS Gc的内存使用率: _initiating_occupancy=%lf.\n", __FILE__, __LINE__, pthread_self(), this->name(), _initiating_occupancy);
}

/**
 * 创建年老代+永久代的软/弱引用分析处理器
 */
  ...
  _intra_sweep_estimate(CMS_SweepWeight, CMS_SweepPadding)
{
  ...
  if (CMSConcurrentMTEnabled) {	//Gc时多线程工作模式

    if (FLAG_IS_DEFAULT(ConcGCThreads)) {	//CMS Gc工作线程的数量是默认配置,则使用ParallelGCThreads来调整
      // just for now
      FLAG_SET_DEFAULT(ConcGCThreads, (ParallelGCThreads + 3)/4);
    }

    if (ConcGCThreads > 1) {
      _conc_workers = new YieldingFlexibleWorkGang("Parallel CMS Threads", ConcGCThreads, true);
      if (_conc_workers == NULL) {
        warning("GC/CMS: _conc_workers allocation failure: forcing -CMSConcurrentMTEnabled");
        CMSConcurrentMTEnabled = false;
        printf("%s[%d] [tid: %lu]: CMS Gc过程中的多线程工作模式设置失败.\n", __FILE__, __LINE__, pthread_self());
      } else {
    	printf("%s[%d] [tid: %lu]: 试图创建并初始化CMS Gc过程中的工作线程工...\n", __FILE__, __LINE__, pthread_self());
        _conc_workers->initialize_workers();
      }
    } else {
      printf("%s[%d] [tid: %lu]: 没有配置多个CMS Gc工作线程,所以自动转变为非并发工作模式.\n", __FILE__, __LINE__, pthread_self());
      CMSConcurrentMTEnabled = false;
    }
  }
  ...
  // are not shared with parallel scavenge (ParNew).
  {
    ...
    if ((CMSParallelRemarkEnabled || CMSConcurrentMTEnabled
      ...
      printf("%s[%d] [tid: %lu]: 试图创建%u个CMS Gc任务队列...\n", __FILE__, __LINE__, pthread_self(), num_queues);

      _task_queues = new OopTaskQueueSet(num_queues);

      if (_task_queues == NULL) {
        warning("task_queues allocation failure.");
        printf("%s[%d] [tid: %lu]: 创建%u个CMS Gc任务队列失败!\n", __FILE__, __LINE__, pthread_self(), num_queues);
        return;
      }
      ...
      printf("%s[%d] [tid: %lu]: 试图创建%u个CMS Gc工作队列...\n", __FILE__, __LINE__, pthread_self(), num_queues);
  ...
  printf("%s[%d] [tid: %lu]: 试图创建一个CMS Gc后台触发工作线程: [Concurrent Mark-Sweep GC Thread]...\n", __FILE__, __LINE__, pthread_self());
  ...
  GenCollectedHeap* gch = GenCollectedHeap::heap();
  _young_gen = gch->prev_gen(_cmsGen);	//获取年青代内存管理器
 ...
 * 当前内存代中第一个可压缩的内存区
 */
CompactibleSpace* ConcurrentMarkSweepGeneration::first_compaction_space() const {
  return _cmsSpace;
}
...
/**
 * 重新计算/调整当前内存代的大小
 * 	1.如果当前发生了增量式Gc失败,则扩展当前内存代的内存容量
 * 	2.如果当前内存代的实际空闲率小于配置允许的最小空闲率,则扩展内存代的存储容量
 */
void ConcurrentMarkSweepGeneration::compute_new_size() {
  assert_locked_or_safepoint(Heap_lock);

  printf("%s[%d] [tid: %lu]: 开始重新计算/调整当前内存代[%s]的大小...\n", __FILE__, __LINE__, pthread_self(), this->name());

  //发生了增量式Gc失败,则直接扩展内存容量
  ...
  /**
   * 根据内存代配置的最小/大空闲率来调整其容量
   */

  size_t expand_bytes = 0;	//当前内存代需要扩展的内存大小
  double free_percentage = ((double) free()) / capacity();	//当前内存代的空闲率
  double desired_free_percentage = (double) MinHeapFreeRatio / 100;	//当前内存代配置允许的最小空闲率
  double maximum_free_percentage = (double) MaxHeapFreeRatio / 100; //当前内存代配置允许的最大空闲率

  //当前内存代的实际空闲率小于配置允许的最小空闲率,则扩展内存代的存储容量
  ...
  if (expand_bytes > 0) {
    ...
    printf("%s[%d] [tid: %lu]: 试图扩展当前内存代[%s]的容量[%lu----->%lu]...\n", __FILE__, __LINE__, pthread_self(),
    		this->name(), capacity(), capacity()+expand_bytes);
...
/**
 * 从当前内存代分配指定大小的内存块(串行分配---内部加锁实现多线程安全)
 */
 ...
 * 对升级过来的老对象进行脏对象标记处理,
 * 如果当前的Gc过程已经执行完了初始标记阶段,则因该标记该对象标记
 * 如果当前的Gc过程在[并发标记, 并发收集)阶段,则因该将该对象标记为脏对象
 */
void CMSCollector::promoted(bool par, HeapWord* start,
  ...
  if (_collectorState >= Marking) {	//当前的Gc过程已经执行完了初始标记阶段
    ...
    if (_collectorState < Sweeping) {	//当前的Gc过程还没有执行到清理阶段,则将该对象标记为脏对象
      ...
      if (is_obj_array) {	//数组类型的对象
        ...
        // card size.
        MemRegion mr(start, (HeapWord*)round_to((intptr_t)(start + obj_size),
...
{
  Generation* gen0 = GenCollectedHeap::heap()->get_gen(0);		//年青代
  EdenSpace* eden = gen0->as_DefNewGeneration()->eden();		//年青代中的Eden区
 ...
 * 确定年青代中Eden区的新逻辑边界
 */
HeapWord* CMSCollector::allocation_limit_reached(Space* space, HeapWord* top,
                                       size_t word_size)
{
  // A start_limit equal to end() means the duty cycle is 0, so treat that as a
  // nop.
  if (CMSIncrementalMode && _icms_start_limit != space->end()) {//配置了后台增量式CMS Gc模式
    if (top <= _icms_start_limit) {
      ...
      printf("%s[%d] [tid: %lu]: 试图开启后台增量式CMS Gc...\n", __FILE__, __LINE__, pthread_self());
      ...
      printf("%s[%d] [tid: %lu]: 试图停止后台增量式CMS Gc...\n", __FILE__, __LINE__, pthread_self());
      //年青代的软限制达到了其上界,则需要停止后台增量式CMS Gc
    ...
    if (CMSTraceIncrementalMode) {	//打印增量式CMS Gc的信息
      space->print_on(gclog_or_tty);
      gclog_or_tty->stamp();
...
oop ConcurrentMarkSweepGeneration::promote(oop obj, size_t obj_size) {
...
#ifndef PRODUCT
  if (Universe::heap()->promotion_should_fail()) {	//对象升级将失败
    return NULL;
  }
#endif  // #ifndef PRODUCT

  //将老对象转存储到当前内存代的核心内存区中
  oop res = _cmsSpace->promote(obj, obj_size);

  if (res == NULL) {	//转存储失败则扩张当前内存代核心内存区的容量再尝试存储

	printf("%s[%d] [tid: %lu]: 试图扩张当前内存代[%s]核心内存区的物理容量...\n", __FILE__, __LINE__, pthread_self(), this->name());

    // expand and retry
    size_t s = _cmsSpace->expansionSpaceRequired(obj_size);  // HeapWords
    expand(s*HeapWordSize, MinHeapDeltaBytes, CMSExpansionCause::_satisfy_promotion);
  ...
  if (res != NULL) {
    ...
    NOT_PRODUCT(
      _numObjectsPromoted++;
      _numWordsPromoted += (int)(CompactibleFreeListSpace::adjustObjectSize(obj->size()));
    )
  }
...
/**
 * 来自年青代的通知:其内存分配达到了内存软限制,裁决其新的分配软限制位置
 *
 * 确定年青代中Eden区的新逻辑(软)边界
 */
HeapWord* ConcurrentMarkSweepGeneration::allocation_limit_reached(Space* space,
                                             HeapWord* top,
                                             size_t word_sz)
{
...
/**
 * 存储年青代升级过来的老对象(并发式)
 * 从指定的Gc线程的本地分配区中分配对象来存储升级过来的老对象
 */
oop ConcurrentMarkSweepGeneration::par_promote(int thread_num,
                                           oop old, markOop m,
                                           size_t word_sz) {
#ifndef PRODUCT
  if (Universe::heap()->promotion_should_fail()) {	//对象升级将失败,则直接返回
    return NULL;
  }
#endif  // #ifndef PRODUCT
  ...
  if (UseCompressedOops) {	//对象压缩存储
    // Copy gap missed by (aligned) header size calculation below
    obj->set_klass_gap(old->klass_gap());
  }

  if (word_sz > (size_t)oopDesc::header_size()) {	//复制旧对象中的数据到新对象中
 ...
 * 不支持并发式老对象升级时的内存分配回滚
 */
void ConcurrentMarkSweepGeneration::par_promote_alloc_undo(int thread_num,
                       HeapWord* obj, size_t word_sz) {
  // CMS does not support promotion undo.
  ShouldNotReachHere();
}

void ConcurrentMarkSweepGeneration::par_promote_alloc_done(int thread_num) {
  CMSParGCThreadState* ps = _par_gc_thread_states[thread_num];
  ps->lab.retire(thread_num);
}

void ConcurrentMarkSweepGeneration::par_oop_since_save_marks_iterate_done(int thread_num) {
  CMSParGCThreadState* ps = _par_gc_thread_states[thread_num];
...
/**
 * 判断当前是否应该进行Gc操作
 * 	1.应用层Gc调用(System.gc()等)
 * 	2.用户自定义的内存使用率触发Gc被禁止:
 * 		1).
 * 		2).当前年老代内存使用率>CMSBootstrapOccupancy
 * 	3.增量式Gc失败
 * 	4.年老代当前需要Gc
 * 	5.永久代当前需要Gc
 */
...
void ConcurrentMarkSweepGeneration::collect(bool   full,
                                            ...
                                            bool   tlab)
{
  printf("%s[%d] [tid: %lu]: 开始试图对内存代[%s]执行一次Gc...\n", __FILE__, __LINE__, pthread_self(), this->name());
                           ...
                           bool   tlab)
{
  if (!UseCMSCollectionPassing && _collectorState > Idling) {
	...
	printf("%s[%d] [tid: %lu]: 放弃对(年老+永久)代执行Gc.\n", __FILE__, __LINE__, pthread_self());
  ...
  if (GC_locker::is_active()) {	//其它线程正在执行Gc
 ...
 // printf("%s[%d] [tid: %lu]: 放弃对内存代[%s]执行Gc.\n", __FILE__, __LINE__, pthread_self());
...
void CMSCollector::acquire_control_and_collect(bool full,
  ...
  assert(!Thread::current()->is_ConcurrentGC_thread(), "shouldn't try to acquire control from self!");
...

  {
    ...
    if (_foregroundGCShouldWait) {
      ...
      ConcurrentMarkSweepThread::clear_CMS_flag(ConcurrentMarkSweepThread::CMS_vm_has_token);  // release token
...
/**
 * 决策本次Gc是否需要整理碎片或者从头开始执行
 * CMS方式很容易产生碎片，所以JVM提供了一个整理内存碎片的动能，由UseCMSCompactAtFullCollection开启
 *   1.Full Gc的次数>CMSFullGCsBeforeCompaction
 *   2.应用层Gc调用
 *   3.增量式Gc失败
 *   4.配置了当清除所有的软引用时进行压缩(CMSCompactWhenClearAllSoftRefs) + 本次Gc需要清除软引用
 */
void CMSCollector::decide_foreground_collection_type(
  bool clear_all_soft_refs, bool* should_compact, bool* should_start_over) {
  ...
  if (clear_all_soft_refs && !*should_compact) {
    ...
    if (CMSCompactWhenClearAllSoftRefs) {
      ...
      *should_compact = true;
      ...
      //本次Gc被要求清理所有的软引用对象,但又无法进行标记-清除-压缩式Gc,所以，如果当前的CMS Gc过了最终标记阶段,
      //则CMS Gc应该重做
...
/**
 * 后台CMS Gc的执行
 * 后台CMS Gc线程检测触发的Gc(包括CMSIncrementalMode下的增量式Gc);
 * 如果禁止了后台CMS Gc则忽略本次CMS Gc请求()
 */
void CMSCollector::collect_in_background(bool clear_all_soft_refs) {
  ...
  GenCollectedHeap* gch = GenCollectedHeap::heap();
  {
    ...
    //前台CMS Gc正在进行或不允许执行后台CMS Gc,所以忽略本次后台的CMS Gc
  ...
  while (_collectorState != Idling) {
...

    {
      ...
      if (waitForForegroundGC()) {	//前台的CMS Gc正在进行,所以忽略本次后台的CMS Gc
        ...
        if (_collectorState == Idling) {	//前台的CMS Gc刚刚完成,所以忽略本次后台的CMS Gc
          break;
        }
      }
    }

    //确保前台CMS Gc在触发时必须等待后台CMS Gc完成
    assert(_foregroundGCShouldWait, "Foreground collector, if active, "
      "should be waiting");

    switch (_collectorState) {
      case InitialMarking:		//初始标记阶段
        {
        ...
        break;
      case Marking:	//第一次并发标记阶段
        // initial marking in checkpointRootsInitialWork has been completed
        if (markFromRoots(true)) {
...
void CMSCollector::collect_in_foreground(bool clear_all_soft_refs) {
  ...
  while (_collectorState != Idling) {
    ...
    switch (_collectorState) {
      case InitialMarking:					//初始标记(根对象)阶段
...
// below.
/**
 * 本次CMS Gc是否也回收永久代(卸载无用的类类型),判断条件(||)
 * 	1.应用层强制Gc调用&&命令行参数配置ExplicitGCInvokesConcurrentAndUnloadsClasses
 * 	2.三个条件满足之一即可(||):
 * 		a.非永久代CMS Gc连续次数>=CMSClassUnloadingMaxInterval
 * 		b.永久代当前需要进行垃圾回收
 * 		c.年老代占用太满
 */
 ...
 * 当前内存代的占用太多,判断标准(&):
 * 	1.当前内存代已经可以进行CMS Gc;
 * 	2.当前内存代的使用率>CMSIsTooFullPercentage
 */
...
/**
 * 以可扩展内存代容量方式从指定的Gc线程本地分配区中分配指定大小的内存块(并发式)
 */
HeapWord* ConcurrentMarkSweepGeneration::expand_and_par_lab_allocate(CMSParGCThreadState* ps, size_t word_sz) {
  ...
  while (true) {
    ...
    //当前JVM没有足够的空闲内存空间来扩展当前内存代
    if (_virtual_space.uncommitted_size() < (word_sz * HeapWordSize)) {
      return NULL;
    }

    // Otherwise, we try expansion.
    expand(word_sz*HeapWordSize, MinHeapDeltaBytes, CMSExpansionCause::_allocate_par_lab);
...
/**
 * 第一次标记(初始标记阶段),只标记存储在旧生代和永久代中的根对象
 */
void CMSCollector::checkpointRootsInitial(bool asynch) {
  ...
  printf("%s[%d] [tid: %lu]: 开始第一次标记年老代+永久代中的根对象(初始标记阶段)...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * 标记旧生代和永久代中的根对象,并通过位图来记录这些标记的对象
 */
...
bool CMSCollector::markFromRootsWork(bool asynch) {
  ...
  bool result = false;
  if (CMSConcurrentMTEnabled && ConcGCThreads > 0) {	//JVM配置了多个Gc线程
...
void CMSCollector::sweepWork(ConcurrentMarkSweepGeneration* gen,
  ...
  {
    SweepClosure sweepClosure(this, gen, &_markBitMap, CMSYield && asynch);
...
/**
 * 完成一个JVM级操作,主要有两种操作类型
 * 	1.CMS Gc过程中的初始标记(第一次串行标记)
 * 	2.CMS Gc过程中的重新标记(第二次串行标记)
 */
void CMSCollector::do_CMS_operation(CMS_op_type op) {
  ...
  switch (op) {
    case CMS_op_checkpointRootsInitial: {	//初始标记(第一次串行标记)
      SvcGCMarker sgcm(SvcGCMarker::OTHER);

      printf("%s[%d] [tid: %lu]: 试图对旧生代[永久代]进行一次异步的初始标记...\n", __FILE__, __LINE__, pthread_self());
    ...
    case CMS_op_checkpointRootsFinal: {		//重新标记(第二次串行标记)
      SvcGCMarker sgcm(SvcGCMarker::OTHER);

      printf("%s[%d] [tid: %lu]: 试图对旧生代[永久代]进行一次异步的重新标记...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * 标记指定的对象及其引用对象
 * 	CMS Gc过程中的第二次标记,本次标记是在一次checkpoint中进行的,即暂停应用
 */
void MarkRefsIntoAndScanClosure::do_oop(oop obj) {
  if (obj != NULL) {
    ...
    /**
     * 指定的对象在当前的内存空间中并且还没有被标记
     */
    if (_span.contains(addr) && !_bit_map->isMarked(addr)) {
      // mark bit map (object is now grey)
      _bit_map->mark(addr);	//标记该对像

      // 将标记对象放入栈中,并标记处理当前栈中所有对象的引用对象
...
/**
 * 标记处理指定的对象
 * 如果指定的对象内存空间位置在当前对象标记位图扫描位置之前,则将其放入标记对象存储栈中，随后标记处理其
 * 引用对象;否则等到对象标记位图扫描到该对象时再标记处理其引用对象
 *
 */
...
size_t SweepClosure::do_blk_careful(HeapWord* addr) {
  ...
  if (addr >= _limit) { // we have swept up to or past the limit: finish up
    assert(_limit >= _sp->bottom() && _limit <= _sp->end(), "sweep _limit out of bounds");
  ...
  } else if (!_bitMap->isMarked(addr)) {	//当前内存块前面部分空闲
...
void SweepClosure::do_already_free_chunk(FreeChunk* fc) {
  ...
  if (!fc->cantCoalesce()) {
    ...
    if (_sp->adaptive_freelists()) {
      ...
      if (!inFreeRange()) {	//前一个内存块被使用
        ...
        //如果前一个内存块还在free list中,则将其也从free list中删除以便合并
        if (freeRangeInFreeLists()) {
  ...
  } else {	//当前内存块不允许和前面临接的空闲块练接,则前面的空闲块放入list free表中
    // Code path common to both original and adaptive free lists.

    //前面内存块是空闲的,则将其放入list free表中
    if (inFreeRange()) {
...
/**
 * 处理指定内存块前面空闲的部分
 * 	1.如果该空闲部分和前一个空闲块是临接者的，则将前一个内存块从free list移出以合并
 */
size_t SweepClosure::do_garbage_chunk(FreeChunk* fc) {
  ...
  if (_sp->adaptive_freelists()) {
    ...
    if (!inFreeRange()) {	//上一块内存区被使用,所以将当前内存区设置为空闲
      ...
      //将上一块空闲内存区从free list中删除,以便和紧接着的下一个空闲内存块连接成一个大空闲内存块
      //以减少内存碎片
      if (freeRangeInFreeLists()) {
        FreeChunk* ffc = (FreeChunk*)freeFinger();
        assert(ffc->size() == pointer_delta(addr, freeFinger()),
...
size_t SweepClosure::do_live_chunk(FreeChunk* fc) {
  ...
  if (_bitMap->isMarked(addr + 1)) {	//当前内存块有连续的使用对象
...
void SweepClosure::do_post_free_or_garbage_chunk(FreeChunk* fc,
  ...
  switch (FLSCoalescePolicy) {
    ...
    case 3: { // coalesce if left OR right chunk on overpopulated list
      coalesce = _sp->coalOverPopulated(left) || _sp->coalOverPopulated(right);
      break;
    }
  ...
  if (doCoalesce) {
    ...
    if (inFreeRange()) {
      ...
      flush_cur_free_chunk(freeFinger(), pointer_delta(fc_addr, freeFinger()));
    }
...
void SweepClosure::lookahead_and_flush(FreeChunk* fc, size_t chunk_size) {
  ...
  if (eob >= _limit) {	//指定内存块是当前内存代的核心内存区的最后一个块
...
/**
 * 将指定的空闲内存块放回到当前内存代的核心内存区的free list表中
 */
void SweepClosure::flush_cur_free_chunk(HeapWord* chunk, size_t size) {
  assert(inFreeRange(), "Should only be called if currently in a free range.");
  assert(size > 0, "A zero sized chunk cannot be added to the free lists.");

  if (!freeRangeInFreeLists()) {	//空闲内存块不在free list表中
    if (CMSTestInFreeList) {
      FreeChunk* fc = (FreeChunk*) chunk;
      fc->setSize(size);
      assert(!_sp->verifyChunkInFreeLists(fc), "chunk should not be in free lists yet");
    }
    ...
    //将当前空闲的内存块放入当前内存代的核心内存区的free list中
    _sp->addChunkAndRepairOffsetTable(chunk, size, lastFreeRangeCoalesced());

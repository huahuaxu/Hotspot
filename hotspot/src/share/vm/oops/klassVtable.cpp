...
bool klassVtable::needs_new_vtable_entry(methodHandle target_method,
  ...
  // a new entry
  ResourceMark rm;
  Symbol* name = target_method()->name();	//方法名字
  Symbol* signature = target_method()->signature();	//方法签名
  klassOop k = super;
...
/**
 * 判断给定的类及其所有的祖先父类是否实现了该接口方法，如果实现了放回false，否则放回ture
 */
bool klassVtable::is_miranda(methodOop m, objArrayOop class_methods, klassOop super) {
  Symbol* name = m->name();	//函数名
  Symbol* signature = m->signature();	//函数签名
...
void klassVtable::add_new_mirandas_to_list(GrowableArray<methodOop>* list_of_current_mirandas,
  ...
  for (int i = 0; i < num_methods; i++) {
    ...
    if (!is_duplicate) { // we don't want duplicate miranda entries in the vtable
      if (is_miranda(im, class_methods, super)) { //如果当前类及其父类没有实现该接口方法
        instanceKlass *sk = instanceKlass::cast(super);
        //检查该接口方法是否出现在父类的接口方法中
...
/**
 * 提取出没有被该类实现的所有接口方法
 * 很明显，如果该类的某一祖先类实现了该接口方法，则该类也就实现了该接口方法
 */
...
void visit_all_interfaces(objArrayOop transitive_intf, InterfaceVisiterClosure *blk) {
  ...
  for(int i = 0; i < transitive_intf->length(); i++) {
    ...
    assert(Klass::cast(intf)->is_interface(), "sanity check");	//确保该类型是一个接口类型
...
/**
 * 统计至少声明了一个方法的接口数量及所有接口方法的总数量，并计算它们占用的空间大小
 */

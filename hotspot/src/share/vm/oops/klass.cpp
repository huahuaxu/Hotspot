...
KlassHandle Klass::base_create_klass(KlassHandle& klass, int size, const Klass_vtbl& vtbl, TRAPS) {
...
void* Klass_vtbl::operator new(size_t ignored, KlassHandle& klass, int size, TRAPS) {
  ...
  klassOop k = (klassOop) CollectedHeap::permanent_obj_allocate_no_klass_install(klass, size, CHECK_NULL);
  return k->klass_part();
}

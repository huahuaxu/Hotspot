...
/**
 * 构造方法的全限定名：类全限定名.方法名.方法签名，并将其存储到指定的char[]中
 * 静态方法
 */
...
/**
 * 获取当前方法的返回值类型:	(boolean,char,float,double,byte,short,int,long,object,array)
 */
...
void methodOopDesc::set_native_function(address function, bool post_event_flag) {
  ...
  address* native_function = native_function_addr();//该本地方法的原入口地址

...
class CollectorPolicy : public CHeapObj {
 protected:
  ...
  size_t _initial_heap_byte_size;	//内存堆的初始大小
  size_t _max_heap_byte_size;		//内存堆的初最大大小
  size_t _min_heap_byte_size;		//内存堆的初最小大小

  size_t _min_alignment;
  size_t _max_alignment;

  // The sizing of the heap are controlled by a sizing policy.
  AdaptiveSizePolicy* _size_policy;		//内存堆大小调整策略
 ...
 public:
  ...
  /**
   * 从内存堆中分配指定大小的内存块
   * 该方法将控制如何从内存堆中分配策略(包括如何触发Gc),gc_overhead_limit_was_exceeded将保存本次分配过程总发生的
   * Gc耗时是否过长
   */

                               ...
                               MinHeapDeltaBytes, level, remset, NULL)
{
  ...
  if (UseParNewGC && ParallelGCThreads > 0) {
    ...
    _alloc_buffers = NEW_C_HEAP_ARRAY(ParGCAllocBufferWithBOTPtr, ParallelGCThreads);
    if (_alloc_buffers == NULL)
      vm_exit_during_initialization("Could not allocate alloc_buffers");

    for (uint i = 0; i < ParallelGCThreads; i++) {
      _alloc_buffers[i] = new ParGCAllocBufferWithBOT(OldPLABSize, _bts);
...
/**
 * 当前内存代Gc之前的前置处理:
 * 	1.记录GC之前内存代的使用量及容量(用于Gc之后重置内存代的初始大小)
 */
...
void TenuredGeneration::collect(bool   full,
  ...
  printf("%s[%d] [tid: %lu]: 开始对内存代[%s]进行垃圾回收...\n", __FILE__, __LINE__, pthread_self(), this->name());

  retire_alloc_buffers_before_full_gc();

  OneContigSpaceCardGeneration::collect(full, clear_all_soft_refs, size, is_tlab);
}
...
oop TenuredGeneration::par_promote(int thread_num,
  ...
  if (obj_ptr == NULL) {
    ...
    if (word_sz * 100 < ParallelGCBufferWastePct * buf->word_sz()) {
      ...
      HeapWord* buf_space = TenuredGeneration::par_allocate(buf_size, false);
      if (buf_space == NULL) {

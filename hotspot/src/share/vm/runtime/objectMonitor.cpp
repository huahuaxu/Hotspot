...
void ATTR ObjectMonitor::enter(TRAPS) {
  ...
  { // Change java thread status to indicate blocked on monitor enter.
    ...
    for (;;) {
      ...
      if (!ExitSuspendEquivalent(jt)) break ;	//结束了自己的等待状态
...
int ObjectMonitor::TryLock (Thread * Self) {
   for (;;) {
      ...
      if (Atomic::cmpxchg_ptr (Self, &_owner, NULL) == NULL) {	//当前线程成功持有对象锁
...
void ATTR ObjectMonitor::EnterI (TRAPS) {
    ...
    for (;;) {
        node._next = nxt = _cxq ;
        if (Atomic::cmpxchg_ptr (&node, &_cxq, nxt) == nxt) break ; //当前线程将自己放到等待队列中

        // Interference - the CAS failed because _cxq changed.  Just retry.
        // As an optional optimization we retry the lock.
        if (TryLock (Self) > 0) {	//当前线程成功持有该对象锁
            assert (_succ != Self         , "invariant") ;
            assert (_owner == Self        , "invariant") ;
            assert (_Responsible != Self  , "invariant") ;
            return ;
    ...
    for (;;) {
        ...
        if(TryLock (Self) > 0) break ;	//当前线程成功持有该对象锁
        assert (_owner != Self, "invariant") ;
        ...
        if (_Responsible == Self || (SyncFlags & 1)) {
        	...
        	printf("%s[%d] [tid: %lu]: 当前线程[%s]开始休眠 %d ms...\n", __FILE__, __LINE__, pthread_self(), Self->name(), RecheckInterval);
...
/**
 * 当前线程释放对象锁，并将该对象锁交给等待的线程
 */
void ATTR ObjectMonitor::exit(TRAPS) {
   Thread * Self = THREAD ;	//当前线程
   ...
   for (;;) {
      ...
      if (Knob_ExitPolicy == 0) {
         ...
         OrderAccess::release_store_ptr (&_owner, NULL) ;   //直接释放锁
         OrderAccess::storeload() ;                         // See if we need to wake a successor
         //没有任何等待对象锁或可唤醒的线程，或者对象锁被刚刚新来的线程占有，则没必要从两个队列唤醒任何一个等待线程
         ...
         //检查当前对象锁是否已经被其它线程持有，若是则直接返回，否则则先暂时持有该对象锁
         if (Atomic::cmpxchg_ptr (THREAD, &_owner, NULL) != NULL) {
            return ;
         }
         TEVENT (Exit - Reacquired) ;
    	  ...
    	  //没有任何等待对象锁或可唤醒的线程，或者对象锁被刚刚新来的线程占有，则没必要从两个队列唤醒任何一个等待线程
         if ((intptr_t(_EntryList)|intptr_t(_cxq)) == 0 || _succ != NULL) {
            OrderAccess::release_store_ptr (&_owner, NULL) ;   //当前线程释放对象锁
            OrderAccess::storeload() ;

            //没有任何等待对象锁的线程，或者对象锁被刚刚新来的线程占有，则没必要从两个队列唤醒任何一个等待线程
      ...
      //优先将对象锁交给可以被唤醒的线程(将一个等待对象锁的线程挂到可以被唤醒的线程队列的尾部)
      ...
      //优先将对象锁交给等待对象锁的线程(将当前所有等待对象锁的线程挂到可以被唤醒的线程队列的头部)
      ...
      //此时可以被唤醒的线程队列为空，将所有的等待对象锁线程反向挂载到可以被唤醒的线程队列
      if (QMode == 1) {
      ...
      } else {	//此时可以被唤醒的线程队列为空，将所有的等待对象锁线程正向挂载到可以被唤醒的线程队列
         // QMode == 0 or QMode == 2
         _EntryList = w ;
         ObjectWaiter * q = NULL ;
         ObjectWaiter * p ;
...
bool ObjectMonitor::ExitSuspendEquivalent (JavaThread * jSelf) {
   ...
   if (Mode && !jSelf->is_external_suspend()) {
      ...
      if (!jSelf->is_external_suspend()) return false ;	//当前线程目前没有被其它线程中断
...
void ObjectMonitor::ExitEpilog (Thread * Self, ObjectWaiter * Wakee) {
   ...
   printf("%s[%d] [tid: %lu]: 开始唤醒线程[%s]...\n", __FILE__, __LINE__, pthread_self(), Trigger->_getThread()->name());
   Trigger->unpark() ;
...
void ObjectMonitor::wait(jlong millis, bool interruptible, TRAPS) {
   ...
   assert (Self->_Stalled == 0, "invariant") ;
   Self->_Stalled = intptr_t(this) ;
   jt->set_current_waiting_monitor(this);//设置当前线程正在等待的监控对象
   ...
   printf("%s[%d] [tid: %lu]: 当前线程[%s]开始释放对象锁...\n", __FILE__, __LINE__, pthread_self(), Self->name());
   exit (Self) ;                    // exit the monitor
   guarantee (_owner != Self, "invariant") ;
   ...
   { // State transition wrappers
     ...
     OSThreadWaitState osts(osthread, true);
     {
       ...
       if (interruptible && (Thread::is_interrupted(THREAD, false) || HAS_PENDING_EXCEPTION)) {
         ...
         if (millis <= 0) {
        	printf("%s[%d] [tid: %lu]: 当前线程[%s]开始等待被唤醒...\n", __FILE__, __LINE__, pthread_self(), jt->name());
            Self->_ParkEvent->park () ;
         } else {
        	printf("%s[%d] [tid: %lu]: 当前线程[%s]开始等待 %ld ms...\n", __FILE__, __LINE__, pthread_self(), jt->name(), millis);
            ret = Self->_ParkEvent->park (millis) ;
         }
       }

   	   printf("%s[%d] [tid: %lu]: 当前线程[%s]已经被唤醒...\n", __FILE__, __LINE__, pthread_self(), jt->name());

       // were we externally suspended while we were waiting?
       //如果当前线程被其它线程暂停，则立即挂起该线程
     ...
     if (v == ObjectWaiter::TS_RUN) {
     	 printf("%s[%d] [tid: %lu]: 当前线程[%s]试图重新占用对象锁...\n", __FILE__, __LINE__, pthread_self(), Self->name());
         enter (Self) ;	//开始试图重新占用该对象锁
     } else {
         guarantee (v == ObjectWaiter::TS_ENTER || v == ObjectWaiter::TS_CXQ, "invariant") ;
         ReenterI (Self, &node) ;
   ...
   //当前线程不是被其他线程唤醒的，所以检查是不是有来自其它线程的中断命令
   if (!WasNotified) {
...
void ObjectMonitor::notify(TRAPS) {
  ...
  if (iterator != NULL) {
     ...
     if (Policy == 0) {       //追加到可以被唤醒的线程队列的头部
     ...
     } else if (Policy == 1) {      //追加到可以被唤醒的线程队列的尾部
         if (List == NULL) {
             iterator->_next = iterator->_prev = NULL ;
             _EntryList = iterator ;
         } else {
     ...
     } else if (Policy == 2) {      //追加到等待对象锁的线程队列的头部
         // prepend to cxq
         if (List == NULL) {
             iterator->_next = iterator->_prev = NULL ;
             _EntryList = iterator ;
     ...
     } else if (Policy == 3) {      //追加到等待对象锁的线程队列的尾部
        iterator->TState = ObjectWaiter::TS_CXQ ;
        for (;;) {
            ObjectWaiter * Tail ;
            Tail = _cxq ;
        ...
        OrderAccess::fence() ;
    	printf("%s[%d] [tid: %lu]: 开始唤醒线程[%s]...\n", __FILE__, __LINE__, pthread_self(), ev->_getThread()->name());
        ev->unpark() ;
     }
...
void ObjectMonitor::Initialize () {
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化JVM的全局对象锁...\n", __FILE__, __LINE__, pthread_self());
...
void ObjectMonitor::DeferredInitialize () {
  ...
  printf("%s[%d] [tid: %lu]: 对象锁的通知策略(Knob_MoveNotifyee=%d; Knob_ExitPolicy=%d; Knob_QMode=%d)..\n", __FILE__, __LINE__, pthread_self(), Knob_MoveNotifyee, Knob_ExitPolicy, Knob_QMode);

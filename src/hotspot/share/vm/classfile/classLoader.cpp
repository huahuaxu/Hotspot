...
// helper routines
/**
 * 判断字符串str是否以给定的字符串str_to_find开头
 */
...
/**
 * 判断字符串str是否以给定的字符串str_to_find结尾
 */
...
MetaIndex::MetaIndex(char** meta_package_names, int num_meta_package_names) {
  if (num_meta_package_names == 0) {
    ...
    _meta_package_names = NEW_C_HEAP_ARRAY(char*, num_meta_package_names); //申请内存空间
...
MetaIndex::~MetaIndex() {
  FREE_C_HEAP_ARRAY(char*, _meta_package_names);//释放申请的内存空间
}
...
/**
 * 从当前的类目录中查找指定的类class文件
 */
ClassFileStream* ClassPathDirEntry::open_stream(const char* name) {

  printf("%s[%d] [tid: %lu]: 试图从目录[%s]中查找并读取class文件[%s]...\n", __FILE__, __LINE__, pthread_self(), _dir, name);
  ...
  if (os::stat(path, &st) == 0) {
    // found file, open it
	printf("%s[%d] [tid: %lu]: 打开类class文件[%s].\n", __FILE__, __LINE__, pthread_self(), path);
    int file_handle = os::open(path, 0, 0);
    if (file_handle != -1) {
      //从当前线程的内存区申请内存来存储该类的二进制字节码
      u1* buffer = NEW_RESOURCE_ARRAY(u1, st.st_size);

      printf("%s[%d] [tid: %lu]: 开始从class文件[%s]中读取该类的所有二进制字节码...\n", __FILE__, __LINE__, pthread_self(), path);
      size_t num_read = os::read(file_handle, (char*) buffer, st.st_size);

      printf("%s[%d] [tid: %lu]: 关闭类class文件[%s].\n", __FILE__, __LINE__, pthread_self(), path);
      ...
      printf("%s[%d] [tid: %lu]: 读取类class文件[%s]时发生错误(未能全部读取)!\n", __FILE__, __LINE__, pthread_self(), path);
    }
  }
  return NULL;
}
...
/**
 * 从当前的jar/zip包中读取一个class文件
 */
ClassFileStream* ClassPathZipEntry::open_stream(const char* name) {

  printf("%s[%d] [tid: %lu]: 试图从包[%s]中查找并读取class文件[%s]...\n", __FILE__, __LINE__, pthread_self(), _zip_name, name);
  ...
  if (ReadMappedEntry == NULL || !(*ReadMappedEntry)(_zip, entry, &buffer, filename)) {
      // mmaped access not available, perhaps due to compression,
      // read contents into resource array
      buffer     = NEW_RESOURCE_ARRAY(u1, filesize); //从当前线程的内存区中申请内存空间来存储该class文件的内容
...
ClassFileStream* LazyClassPathEntry::open_stream(const char* name) {
  if (_meta_index != NULL && !_meta_index->may_contain(name)) {
    return NULL;
  }
...
void ClassLoader::setup_meta_index() {
  ...
  printf("%s[%d] [tid: %lu]: 打开文件[%s]...\n", __FILE__, __LINE__, pthread_self(), meta_index_path);
    ...
    while (fgets(package_name, sizeof(package_name), file) != NULL) {
      ...
      switch(package_name[0]) {
        ...
        case '@':
        {
          ...
          if ((cur_entry != NULL) && (boot_class_path_packages.length() > 0)) {
            if (TraceClassLoading && Verbose) {
              print_meta_index(cur_entry, boot_class_path_packages);
            }

            MetaIndex* index = new MetaIndex(boot_class_path_packages.adr_at(0),boot_class_path_packages.length());
...
void ClassLoader::setup_bootstrap_search_path() {
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化启动类装载器的类搜索路径...\n", __FILE__, __LINE__, pthread_self());
...
void ClassLoader::create_class_path_entry(char *path, struct stat st, ClassPathEntry **new_entry, bool lazy) {
  ...
  if ((st.st_mode & S_IFREG) == S_IFREG) {	//该class路径是zip/jar文件
    // Regular file, should be a zip file
    // Canonicalized filename
    char canonical_path[JVM_MAXPATHLEN];
    if (!get_canonical_path(path, canonical_path, JVM_MAXPATHLEN)) {//规范化文件路径
    ...
    jzfile* zip;
    {
      ...
      printf("%s[%d] [tid: %lu]: 试图打开zip/jar文件[%s:%s]...\n", __FILE__, __LINE__, pthread_self(), path, canonical_path);
      zip = (*ZipOpen)(canonical_path, &error_msg);
    }

    if (zip != NULL && error_msg == NULL) {//文件打开成功
      *new_entry = new ClassPathZipEntry(zip, path);
      ...
      printf("%s[%d] [tid: %lu]: zip/jar文件[%s]打开失败!\n", __FILE__, __LINE__, pthread_self(), path);
...
ClassPathZipEntry* ClassLoader::create_class_path_zip_entry(const char *path) {
  ...
  if (os::stat(path, &st) == 0) {
    if ((st.st_mode & S_IFREG) == S_IFREG) {
      ...
      if(get_canonical_path(orig_path, canonical_path, JVM_MAXPATHLEN)) { //规范化路径
        char* error_msg = NULL;
        jzfile* zip;
        {
          // enable call to C land
          ...
          ThreadToNativeFromVM ttn(thread);
          HandleMark hm(thread);
          printf("%s[%d] [tid: %lu]: 试图打开zip/jar文件[%s:%s]...\n", __FILE__, __LINE__, pthread_self(), orig_path, canonical_path);
          zip = (*ZipOpen)(canonical_path, &error_msg);
        }
...
void ClassLoader::add_to_list(ClassPathEntry *new_entry) {
  if (new_entry != NULL) {
	printf("%s[%d] [tid: %lu]: 为启动类装载器添加一个类搜索路径[%s]...\n", __FILE__, __LINE__, pthread_self(), new_entry->name());
    if (_last_entry == NULL) {
...
void ClassLoader::update_class_path_entry_list(const char *path, bool check_for_duplicates) {
  struct stat st;
...
/**
 * 加载zip动态链接库，以便读取以zip格式压缩的class文件
 */
void ClassLoader::load_zip_library() {
  assert(ZipOpen == NULL, "should not load zip library twice");

  printf("%s[%d] [tid: %lu]: 启动类装载器开始加载JVM所依赖的本地链接库...\n", __FILE__, __LINE__, pthread_self());
  ...
  printf("%s[%d] [tid: %lu]: 启动类装载器试图加载zip动态链接库[%s]..\n", __FILE__, __LINE__, pthread_self(), path);
  void* handle = os::dll_load(path, ebuf, sizeof ebuf);
  if (handle == NULL) {
	printf("%s[%d] [tid: %lu]: 启动类装载加载zip动态链接库[%s]失败!\n", __FILE__, __LINE__, pthread_self(), path);
  ...
  if (ZipOpen == NULL || FindEntry == NULL || ReadEntry == NULL || GetNextEntry == NULL) {
	printf("%s[%d] [tid: %lu]: 启动类装载获取zip动态链接库[%s]中的某些方法指针失败!\n", __FILE__, __LINE__, pthread_self(), path);
...
instanceKlassHandle ClassLoader::load_classfile(Symbol* h_name, TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 启动类装载器试图装载类[%s]...\n", __FILE__, __LINE__, pthread_self(), h_name->as_C_string());
  ...
  int classpath_index = 0;
  {
    ...
    ClassPathEntry* e = _first_entry;
    while (e != NULL) {
      printf("%s[%d] [tid: %lu]: 试图从[%s]中搜索类[%s]的class文件.\n", __FILE__, __LINE__, pthread_self(), e->name(), h_name->as_C_string());
      stream = e->open_stream(name);
      if (stream != NULL) {
    	printf("%s[%d] [tid: %lu]: 1).启动类装载器已查找到类[%s]的二进制字节码的位置.\n", __FILE__, __LINE__, pthread_self(), h_name->as_C_string());
        break;
      }
      e = e->next();
      ++classpath_index;
  ...
  if (stream != NULL) {
	printf("%s[%d] [tid: %lu]: 2).启动类装载器开始解析类[%s]的二进制字节码...\n", __FILE__, __LINE__, pthread_self(), h_name->as_utf8());
    ...
    if (add_package(name, classpath_index, THREAD)) {
	  ...
	  printf("%s[%d] [tid: %lu]: 1).启动类装载器加载类[%s]失败，因为它无法找到该类的二进制字节码的位置.\n", __FILE__, __LINE__, pthread_self(), h_name->as_utf8());
  }
...
void ClassLoader::create_package_info_table(HashtableBucket *t, int length,
  ...
  _package_hash_table = new PackageHashtable(package_hash_table_size, t, number_of_entries);
}
...
void ClassLoader::initialize() {
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化JVM的启动类装载器(bootstrap class loader)...\n", __FILE__, __LINE__, pthread_self());
  ...
  printf("%s[%d] [tid: %lu]: JVM的启动类装载器(bootstrap class loader)初始化完毕！\n", __FILE__, __LINE__, pthread_self());
}

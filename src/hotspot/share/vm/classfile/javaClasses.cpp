...
/**
 * 计算某一个对象属性在其对象实例中的存储偏移位置，如果没有找到则出错
 *
 * dest_offset 存储对象属性在其对象实例中的存储偏移位置
 * klass_oop 对象类型的描述信息
 * name_symbol 属性名
 * signature_symbol 属性的类型描述
 * allow_super 查找范围(类直接声明的属性/类+父类+实现接口)
 */
...
/**
 * 计算某一个对象属性在其对象实例中的存储偏移位置，如果没有找到则忽略
 *
 * dest_offset 存储对象属性在其对象实例中的存储偏移位置
 * klass_oop 对象类型的描述信息
 * name_symbol 属性名
 ...
 * allow_super 查找范围(类直接声明的属性/类+父类+实现接口)
 */
static void compute_optional_offset(int& dest_offset,
                        klassOop klass_oop, Symbol* name_symbol, Symbol* signature_symbol,
                        bool allow_super = false) {
  fieldDescriptor fd;
  instanceKlass* ik = instanceKlass::cast(klass_oop);
...
int java_lang_String::value_offset  = 0;	//java.lang.String的value属性在其实例对象中的存储偏移位置
int java_lang_String::offset_offset = 0;	//
int java_lang_String::count_offset  = 0;	//
int java_lang_String::hash_offset   = 0;	//java.lang.String的hash属性在其实例对象中的存储偏移位置

bool java_lang_String::initialized  = false;

/**
 * 计算java.lang.String的对象属性在其对象实例中的存储偏移位置
 */
void java_lang_String::compute_offsets() {
  assert(!initialized, "offsets should be initialized only once");

  //类java.lang.String的描述信息对象
...
/**
 * 创建一个java.lang.String的实例对象
 *  1). 申请java.lang.String的实例对象内存存储空间
 *  2). 申请char数组的内存存储空间
 *  3). 绑定char数组到java.lang.String实例对象的value属性上
 */
Handle java_lang_String::basic_create(int length, bool tenured, TRAPS) {
  ...
  //绑定char数组到java.lang.String实例对象的value属性上
  obj = h_obj();
...
/**
 * 创建一个java.lang.String实例对象，并将unicode编码的字符串拷贝到String实例对象中
 */
...
/**
 * 将unicode编码的字符串转换为java的java.lang.String实例对象，内存空间在堆上分配
 */
...
/**
 * 将unicode编码的字符串转换为java的java.lang.String对象，内存空间在堆上分配
 */
...
/**
 * 将普通的字符串转换为java的java.lang.String实例对象，java实例对象内存空间在堆上分配
 */
...
static void initialize_static_field(fieldDescriptor* fd, TRAPS) {
  ...
  if (fd->has_initial_value()) {
	printf("%s[%d] [tid: %lu]: 开始初始化类型[%s]的静态属性[%s]...\n", __FILE__, __LINE__, pthread_self(), fd->field_holder()->klass_part()->name()->as_C_string(), fd->name()->as_C_string());
...
/**
 * 为一个非基本类型创建对应的java.lang.Class实例
 */
oop java_lang_Class::create_mirror(KlassHandle k, TRAPS) {
  ...
  if (SystemDictionary::Class_klass_loaded() && (k->oop_is_instance() || k->oop_is_javaArray())) {
	...
	printf("%s[%d] [tid: %lu]: 开始为类型[%s]创建对应的java.lang.Class实例...\n", __FILE__, __LINE__, pthread_self(), k->name()->as_C_string());
    ...
    if (k->oop_is_javaArray()) {
      ...
      printf("%s[%d] [tid: %lu]: 开始初始化类型[%s]的静态属性...\n", __FILE__, __LINE__, pthread_self(), k->name()->as_C_string());
...
/**
 * 在某一普通类型的java.lang.Class实例中配置其类型描述指针
 */
...
/**
 * 在某一数组类型的java.lang.Class实例中配置其类型描述指针
 */
...
/**
 * 设置java.lang.Thread对象实例的属性name值
 * (设置Java级线程名)
 */
...
/**
 * 设置java.lang.Thread对象实例的属性threadStatus值
 * 设置Java级线程当前的状态
 */
...
/**
 * 获取java.lang.ref.SoftReference的静态对象
 */
...
int java_lang_Class::_klass_offset;		//普通类的类型描述指针存储偏移位置
int java_lang_Class::_array_klass_offset;	//数组类的类型描述指针存储偏移位置
...
/**
 * 计算JVM核心类中类型属性/对象属性(静态属性/非静态属性)在其对应的java.lang.Class对象/实例对象中的存储偏移位置
 * 	1.java.lang.Throwable
 * 	2.基本类型的引用类型(java.lang.Boolean/Byte/Character/Short/Integer/Long/Float/Double)
 * 	3.java.lang.ref.Reference/SoftReference
 * 	4.java.lang.ClassLoader
 * 	5.java.lang.System
 * 	6.java.lang.StackTraceElement
 * 	7.java.lang.AssertionStatusDirectives
 */
void JavaClasses::compute_hard_coded_offsets() {
  const int x = heapOopSize;
  const int header = instanceOopDesc::base_offset_in_bytes();

  printf("%s[%d] [tid: %lu]: 开始计算JVM核心类中类型属性/对象属性(静态属性/非静态属性)在其对应的java.lang.Class对象/实例对象中的存储偏移位置...\n", __FILE__, __LINE__, pthread_self());

  //计算java.lang.Throwable中的对象属性在其对象空间中的偏移位置

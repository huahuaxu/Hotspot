...
void ClassFileParser::parse_constant_pool_entries(constantPoolHandle cp, int length, TRAPS) {
  ...
  for (int index = 1; index < length; index++) {
    ...
    u1 tag = cfs->get_u1_fast();//常量池入口的类型标志
    switch (tag) {
      case JVM_CONSTANT_Class :	//对一个类/接口的的符号引用
        {
        ...
        break;
      case JVM_CONSTANT_Fieldref :	//对一个字段的符号引用
        {
        ...
        break;
      case JVM_CONSTANT_Methodref :	//对一个类中声明的方法的符号引用
        {
          cfs->guarantee_more(5, CHECK);  // class_index, name_and_type_index, tag/access_flags
          u2 class_index = cfs->get_u2_fast();
          u2 name_and_type_index = cfs->get_u2_fast();
        ...
        break;
      case JVM_CONSTANT_InterfaceMethodref :	//对一个接口中声明的方法的符号引用
        {
          cfs->guarantee_more(5, CHECK);  // class_index, name_and_type_index, tag/access_flags
          u2 class_index = cfs->get_u2_fast();
          u2 name_and_type_index = cfs->get_u2_fast();
        ...
        break;
      case JVM_CONSTANT_String :	//String类型字面值
        {
      ...
      case JVM_CONSTANT_InvokeDynamic :
        ...
        break;
      case JVM_CONSTANT_Integer :		//int类型字面值
        {
        ...
        break;
      case JVM_CONSTANT_Float :		//float类型字面值
        {
          cfs->guarantee_more(5, CHECK);  // bytes, tag/access_flags
          u4 bytes = cfs->get_u4_fast();
        ...
        break;
      case JVM_CONSTANT_Long :	//dlong类型字面值
        ...
        break;
      case JVM_CONSTANT_Double :	//double类型字面值
        // A mangled type might cause you to overrun allocated memory
        guarantee_property(index+1 < length,
                           "Invalid constant pool entry %u in class file %s",
                           index, CHECK);
        ...
        index++;   // Skip entry following eigth-byte constant, see JVM book p. 98
        break;
      case JVM_CONSTANT_NameAndType :	//对一个字段或方法的部分符号引用
        {
        ...
        break;
      case JVM_CONSTANT_Utf8 :	//UTF-8编码的Unicode字符串
        {
...
constantPoolHandle ClassFileParser::parse_constant_pool(TRAPS) {
  ...
  guarantee_property(length >= 1, "Illegal constant pool size %u in class file %s", length, CHECK_(nullHandle));
  ...
  for (index = 1; index < length; index++) {          // Index 0 is unused
    ...
    switch (tag) {
      case JVM_CONSTANT_Class :   //对一个类或者接口的符号引用
        ShouldNotReachHere();     // Only JVM_CONSTANT_ClassIndex should be present
        break;
      case JVM_CONSTANT_Fieldref :	//对一个字段的引用
        // fall through
      case JVM_CONSTANT_Methodref :	//对一个类中声明的方法的符号引用
        // fall through
      case JVM_CONSTANT_InterfaceMethodref : {	//对一个接口中声明的方法的符号引用
      ...
      case JVM_CONSTANT_Float :		//float类型字面值
        break;
      case JVM_CONSTANT_Long :		//long类型字面值
      case JVM_CONSTANT_Double :	//double类型字面值
        index++;
        check_property(
...
objArrayHandle ClassFileParser::parse_interfaces(constantPoolHandle cp,
  ...
  printf("%s[%d] [tid: %lu]: 试图申请一块永久性内存来存储类[%s]所实现的接口描述的指针.\n", __FILE__, __LINE__, pthread_self(), _class_name->as_C_string());
  ...
  for (index = 0; index < length; index++) {
    ...
    if (cp->tag_at(interface_index).is_klass()) {
      ...
      printf("%s[%d] [tid: %lu]: 试图装载/解析类[%s]所实现的接口[%s].\n", __FILE__, __LINE__, pthread_self(), class_name->as_C_string(), unresolved_klass->as_C_string());
...
typeArrayHandle ClassFileParser::parse_fields(Symbol* class_name,
  ...
  u2 length = cfs->get_u2_fast();	//类所声明的属性数量
  *java_fields_count_ptr = length;
  ...
  printf("%s[%d] [tid: %lu]: 试图申请一块永久性内存来存储类[%s]所声明的属性描述.\n", __FILE__, __LINE__, pthread_self(), _class_name->as_C_string());
...
methodHandle ClassFileParser::parse_method(constantPoolHandle cp, bool is_interface,
  ...
  printf("%s[%d] [tid: %lu]: 试图申请一块内存来存储类[%s]的方法[%s]的描述信息.\n", __FILE__, __LINE__, pthread_self(), _class_name->as_C_string(), name->as_C_string());
...
objArrayHandle ClassFileParser::parse_methods(constantPoolHandle cp, bool is_interface,
  ...
  u2 length = cfs->get_u2_fast();	//声明的方法的数量
  if (length == 0) {
    return objArrayHandle(THREAD, Universe::the_empty_system_obj_array());
  } else {
	printf("%s[%d] [tid: %lu]: 试图申请一块内存来存储类[%s]的方法描述的指针.\n", __FILE__, __LINE__, pthread_self(), _class_name->as_C_string());
...
instanceKlassHandle ClassFileParser::parseClassFile(Symbol* name,
  ...
  if (JvmtiExport::should_post_class_file_load_hook()) {
    ...
    if (state != NULL) {
      KlassHandle *h_class_being_redefined = state->get_class_being_redefined();
  ...
  printf("%s[%d] [tid: %lu]: 类[%s]的class文件的版本号[major=%d, minor=%d].\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), _major_version, _minor_version);
  ...
  int cp_size = cp->length();
  printf("%s[%d] [tid: %lu]: 类[%s]class文件中常量池大小=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), cp_size);

  //访问标志(2)、this_class(2)、super_class(2)、infs_len(2)共占8bytes
  cfs->guarantee_more(8, CHECK_(nullHandle));  // flags, this_class, super_class, infs_len
  ...
  access_flags.set_flags(flags);
  printf("%s[%d] [tid: %lu]: 类[%s]的访问标志=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), flags);
  ...
  { HandleMark hm(THREAD);
    ...
    if (super_class_index == 0) {
      ...
      if (_need_verify) {
        guarantee_property(!is_array, "Bad superclass name in class file %s", CHECK_(nullHandle));
      }
    }
    ...
    printf("%s[%d] [tid: %lu]: 类[%s]实现的父接口数量=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), itfs_len);
    ...
    printf("%s[%d] [tid: %lu]: 类[%s]声明的字段数量=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), java_fields_count);
    ...
    objArrayHandle methods = parse_methods(cp, access_flags.is_interface(),
                                           ...
                                           CHECK_(nullHandle));
    printf("%s[%d] [tid: %lu]: 类[%s]声明的方法数量(类初始化方法+实例初始化方法/构造方法+本地方法+java方法)=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), methods()->length());
    ...
    if (super_class_index > 0 && super_klass.is_null()) {
      ...
      printf("%s[%d] [tid: %lu]: 试图先装载/解析类[%s]的父类[%s].\n", __FILE__, __LINE__, pthread_self(), class_name->as_C_string(), sk->as_C_string());
      klassOop k = SystemDictionary::resolve_super_or_fail(class_name,
                                                           sk,
                                                           class_loader,
                                                           protection_domain,
    ...
    printf("%s[%d] [tid: %lu]: 类[%s]继承的接口总数量=%d.\n", __FILE__, __LINE__, pthread_self(), name->as_C_string(), transitive_interfaces->length());
    ...
    int static_field_size = 0;			//该类型申明的静态属性大小
    int next_static_oop_offset;			//该类型申明的第一个静态类类型属性在内存中的偏移位置
    int next_static_double_offset;		//该类型申明的第一个静态double、long类型属性(类属性)在内存中的偏移位置
    int next_static_word_offset;		//该类型申明的第一个静态float、int类型属性(类属性)在内存中的偏移位置
    int next_static_short_offset;		//该类型申明的第一个静态short、char类型属性(类属性)在内存中的偏移位置
    int next_static_byte_offset;		//该类型申明的第一个静态byte、boolean类型属性(类属性)在内存中的偏移位置
    int next_static_type_offset;		//该类型中静态属性(类属性)存储区的结束位置

    int next_nonstatic_oop_offset;		//该类型的对象中存储第一个非静态类类型属性(对象属性)的偏移位置
    int next_nonstatic_double_offset;	//该类型的对象中存储第一个非静态double、long类型属性(对象属性)的偏移位置
    int next_nonstatic_word_offset;		//该类型的对象中存储第一个非静态float、int类型属性(对象属性)的偏移位置
    int next_nonstatic_short_offset;	//该类型的对象中存储第一个非静态short、char类型属性(对象属性)的偏移位置
    int next_nonstatic_byte_offset;		//该类型的对象中存储第一个非静态byte、boolean类型属性(对象属性)的偏移位置
    int next_nonstatic_type_offset;		//该类型的对象中存储非静态属性(对象属性)的结束偏移位置

    int first_nonstatic_oop_offset;		//
    int first_nonstatic_field_offset;
    int next_nonstatic_field_offset;

    //计算第一个静态非基本类型的属性在内存中的偏移位置
    next_static_oop_offset = instanceMirrorKlass::offset_of_static_fields();
    //计算第一个静态double、long类型的属性在内存中的偏移位置
    next_static_double_offset = next_static_oop_offset + (fac.count[STATIC_OOP] * heapOopSize);
    //对齐
    if ( fac.count[STATIC_DOUBLE] &&
         (Universe::field_type_should_be_aligned(T_DOUBLE) || Universe::field_type_should_be_aligned(T_LONG)) ) {
      next_static_double_offset = align_size_up(next_static_double_offset, BytesPerLong);
    }

    //计算第一个静态非float、int类型的属性在内存中的偏移位置
    next_static_word_offset = next_static_double_offset + (fac.count[STATIC_DOUBLE] * BytesPerLong);
    //计算第一个静态非short、char类型的属性在内存中的偏移位置
    next_static_short_offset = next_static_word_offset + (fac.count[STATIC_WORD] * BytesPerInt);
    //计算第一个静态非byte、boolean类型的属性在内存中的偏移位置
    ...
    //计算本类的第一个非静态属性在内存中的偏移位置(不包括父类的对象属性)
    ...
    //父类是否有对象属性
    bool super_has_nonstatic_fields = (super_klass() != NULL && super_klass->has_nonstatic_fields());
    //类是否有对象属性
    ...
    nonstatic_oop_offsets = NEW_RESOURCE_ARRAY_IN_THREAD(THREAD, int, nonstatic_oop_count + 1);
    nonstatic_oop_counts  = NEW_RESOURCE_ARRAY_IN_THREAD(THREAD, unsigned int, nonstatic_oop_count + 1);

    first_nonstatic_oop_offset = 0; // will be set for first oop field

#ifndef PRODUCT
    if( PrintCompactFieldsSavings ) {
      //计算本类型第一个非静态double、long类型的属性在内存中的偏移位置
      next_nonstatic_double_offset = next_nonstatic_field_offset + (nonstatic_oop_count * heapOopSize);
      if ( nonstatic_double_count > 0 ) {
        next_nonstatic_double_offset = align_size_up(next_nonstatic_double_offset, BytesPerLong);
      }

      //计算本类型第一个非静态float、int类型的属性在内存中的偏移位置
      next_nonstatic_word_offset  = next_nonstatic_double_offset + (nonstatic_double_count * BytesPerLong);
      //计算本类型第一个非静态short、char类型的属性在内存中的偏移位置
      next_nonstatic_short_offset = next_nonstatic_word_offset +(nonstatic_word_count * BytesPerInt);
      //计算本类型第一个非静态byte、boolean类型的属性在内存中的偏移位置
      next_nonstatic_byte_offset  = next_nonstatic_short_offset + (nonstatic_short_count * BytesPerShort);
      //计算本类型非静态属性在内存中的结束位置并对齐
      next_nonstatic_type_offset  = align_size_up((next_nonstatic_byte_offset + nonstatic_byte_count ), heapOopSize );
      //计算非静态属性的大小(包含继承父类的属性)
      orig_nonstatic_field_size   = nonstatic_field_size + ((next_nonstatic_type_offset - first_nonstatic_field_offset)/heapOopSize);
    }
#endif
    ...
    if( allocation_style == 0 ) {
      //类型的对象实例内存布局是：父类属性、对象属性、long/double属性、int/float属性、shor/char属性、byte/boolean属性
      ...
      //类型的对象实例内存布局是：父类属性、long/double属性、int/float属性、shor/char属性、byte/boolean属性、对象属性
      next_nonstatic_double_offset = next_nonstatic_field_offset;
    } else if( allocation_style == 2 ) {
      // Fields allocation: oops fields in super and sub classes are together.
      if( nonstatic_field_size > 0 && super_klass() != NULL && super_klass->nonstatic_oop_map_size() > 0 ) {
    ...
    if( nonstatic_double_count > 0 ) {//本类声明了double、long类型的对象属性
    ...
    printf("%s[%d] [tid: %lu]: 开始计算类型[%s]中的每一个静态属性在其对应的java.lang.Class实例中的偏移位置以及每一个非静态属性在其对应的实例对象中的偏移位置...\n", __FILE__, __LINE__, pthread_self(), class_name->as_C_string());
    for (AllFieldStream fs(fields, cp); !fs.done(); fs.next()) {
      int real_offset;
      FieldAllocationType atype = (FieldAllocationType) fs.offset();
      switch (atype) {
        case STATIC_OOP:	//静态类类型
          real_offset = next_static_oop_offset;
          next_static_oop_offset += heapOopSize;
          break;
        case STATIC_BYTE:	//静态byte、boolean类型
          real_offset = next_static_byte_offset;
          next_static_byte_offset += 1;
          break;
        case STATIC_SHORT:	//静态short、char类型
          real_offset = next_static_short_offset;
          next_static_short_offset += BytesPerShort;
          break;
        case STATIC_WORD:	//静态float、int类型
          real_offset = next_static_word_offset;
          next_static_word_offset += BytesPerInt;
          break;
        case STATIC_DOUBLE:	//静态double、long类型
          real_offset = next_static_double_offset;
          next_static_double_offset += BytesPerLong;
          break;
        case NONSTATIC_OOP:	//非静态类类型
          ...
          break;
        case NONSTATIC_BYTE:	//非静态byte、boolean类型
          ...
          break;
        case NONSTATIC_SHORT:	//非静态short、char类型
          ...
          break;
        case NONSTATIC_WORD:	//非静态float、int类型
          ...
          break;
        case NONSTATIC_DOUBLE:	//非静态double、long类型
    ...
    const unsigned int total_oop_map_count = compute_oop_map_count(super_klass, nonstatic_oop_map_count, first_nonstatic_oop_offset);
    ...
    assert(this_klass->nonstatic_oop_map_count() == total_oop_map_count, "sanity");
    ...
    if (cached_class_file_bytes != NULL) {
      ...
      this_klass->set_cached_class_file(cached_class_file_bytes, cached_class_file_length);
    }
...
void ClassFileParser::set_precomputed_flags(instanceKlassHandle k) {
  ...
  if (!_has_empty_finalizer) {
    if (_has_finalizer || (super != NULL && super->klass_part()->has_finalizer())) {
      k->set_has_finalizer();
    }
  }

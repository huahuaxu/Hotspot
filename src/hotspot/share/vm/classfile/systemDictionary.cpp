...
klassOop SystemDictionary::resolve_or_null(Symbol* class_name, Handle class_loader, Handle protection_domain, TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 试图解析类型[%s]...\n", __FILE__, __LINE__, pthread_self(), class_name->as_C_string());

  if (FieldType::is_array(class_name)) {	//数组对象类型
...
void SystemDictionary::double_lock_wait(Handle lockObject, TRAPS) {
  ...
  bool calledholdinglock = ObjectSynchronizer::current_thread_holds_lock((JavaThread*)THREAD, lockObject);
...
klassOop SystemDictionary::resolve_instance_class_or_null(Symbol* name, Handle class_loader, Handle protection_domain, TRAPS) {
  assert(name != NULL && !FieldType::is_array(name) && !FieldType::is_obj(name), "invalid class name");

  // UseNewReflection
  // Fix for 4474172; see evaluation for more details
  class_loader = Handle(THREAD, java_lang_ClassLoader::non_reflection_class_loader(class_loader()));

  //根据类型的全限定名和类装载器计算其hash值
  unsigned int d_hash = dictionary()->compute_hash(name, class_loader);
  int d_index = dictionary()->hash_to_index(d_hash);
  //先检查该类型是否已经加载了(已存储在系统词典中)，如果已经加载了则直接将其返回
  klassOop probe = dictionary()->find(d_index, d_hash, name, class_loader, protection_domain, THREAD);
  ...
  if (is_parallelCapable(class_loader)) {//类的加载是否允许并行化
    DoObjectLock = false;
  }
  ...
  //该类型还没有被加载过，但在开始加载之前要先加锁以阻止其它线程又加载它(获取该类型装载器的锁)
  Handle lockObject = compute_loader_lock_object(class_loader, THREAD);
  check_loader_lock_contention(lockObject, THREAD);
  ObjectLocker ol(lockObject, THREAD, DoObjectLock);

  // Check again (after locking) if class already exist in SystemDictionary
  bool class_has_been_loaded   = false;	//该类型是否已经被加载
  ...
  {
    MutexLocker mu(SystemDictionary_lock, THREAD);
    //加锁之后再检查该类型是否已经被加载(能获到锁，有可能是该类型刚被加载完而释放了锁)
    klassOop check = find_class(d_index, d_hash, name, class_loader);
    if (check != NULL) {
      //该类型已经被加载
      ...
      placeholder = placeholders()->get_entry(p_index, p_hash, name, class_loader);
      if (placeholder && placeholder->super_load_in_progress()) {	//该类型正在被加载，同时正在加载其父类型
  ...
  if (super_load_in_progress && havesupername==true) {
    k = SystemDictionary::handle_parallel_super_load(name, superclassname, class_loader, protection_domain, lockObject, THREAD);
    if (HAS_PENDING_EXCEPTION) {//出异常了，立即返回
      return NULL;
    }
    if (!k.is_null()) {
      class_has_been_loaded = true;
  ...
  if (!class_has_been_loaded) {
    ...
    // Five cases:
    // All cases need to prevent modifying boot class searchpath
    ...
    bool throw_circularity_error = false;
    {
      ...
      //如果类装载器是启动类装载器或者该类装载器不支持并发
      if (class_loader.is_null() || !is_parallelCapable(class_loader)) {
        PlaceholderEntry* oldprobe = placeholders()->get_entry(p_index, p_hash, name, class_loader);
        if (oldprobe) {
          // only need check_seen_thread once, not on each loop
          // 6341374 java/lang/Instrument with -Xcomp
          //检查当前被加载的类型是否循环依赖,即该类或接口是自身的超类
          if (oldprobe->check_seen_thread(THREAD, PlaceholderTable::LOAD_INSTANCE)) {
        	  printf("%s[%d] [%lu]: 无法加载类[%s],因为该类或接口是自身的超类!\n", __FILE__, __LINE__, pthread_self(), name->as_C_string());
        	  throw_circularity_error = true;
          } else {
      ...
      //当前没有出现类循环依赖错误，且该类/接口还没有被其它线程加载
    ...
    if(!class_has_been_loaded) {
      ...
      if (UnsyncloadClass || (class_loader.is_null())) {
        if (k.is_null() && HAS_PENDING_EXCEPTION && PENDING_EXCEPTION->is_a(SystemDictionary::LinkageError_klass())) {
          MutexLocker mu(SystemDictionary_lock, THREAD);
          klassOop check = find_class(d_index, d_hash, name, class_loader);
          if (check != NULL) {
      ...
      if (!HAS_PENDING_EXCEPTION && !k.is_null() &&  k->class_loader() != class_loader()) {
        ...
        if (!HAS_PENDING_EXCEPTION) {
          { // Grabbing the Compile_lock prevents systemDictionary updates
            ...
            update_dictionary(d_index, d_hash, p_index, p_hash, k, class_loader, THREAD);
          }
...
klassOop SystemDictionary::parse_stream(Symbol* class_name,
  ...
  printf("%s[%d] [tid: %lu]: 开始解析类[%s]的二进制字节码..\n", __FILE__, __LINE__, pthread_self(), class_name->as_C_string());
...
klassOop SystemDictionary::resolve_from_stream(Symbol* class_name,
  ...
  //   to be called for all classes but java.lang.Object; and we preload
  //   java.lang.Object through resolve_or_fail, not this path.
  Symbol* _classloader = Klass::cast(class_loader()->klass())->name();

  if(_classloader) printf("%s[%d] [tid: %lu]: 类装载器[%s]开始解析类[%s]的二进制字节码..\n", __FILE__, __LINE__, pthread_self(), _classloader->as_C_string() ,class_name->as_C_string());
  instanceKlassHandle k = ClassFileParser(st).parseClassFile(class_name,
                                                             class_loader,
                                                             protection_domain,
                                                             parsed_name,
...
instanceKlassHandle SystemDictionary::load_shared_class(Symbol* class_name, Handle class_loader, TRAPS) {
...
instanceKlassHandle SystemDictionary::load_shared_class(instanceKlassHandle ik, Handle class_loader, TRAPS) {
  ...
  if (ik.not_null()) {
    ...
    if (ik->super() != NULL) {
      ...
      resolve_super_or_fail(class_name, cn, class_loader, Handle(), true, CHECK_(nh));
    }
    ...
    ClassLoadingService::notify_class_loaded(instanceKlass::cast(ik()), true /* shared class */);
  }
...
instanceKlassHandle SystemDictionary::load_instance_class(Symbol* class_name, Handle class_loader, TRAPS) {
  ...
  if (class_loader.is_null()) {//当前是启动类装载器在负责加载该类型
    ...
    return k;
  } else {//当前是扩展类转载器、应用类装载器、用户自定义类转载器在负责加载该类型
    ...
    if (MustCallLoadClassInternal && has_loadClassInternal()) {
      if(_classloader) printf("%s[%d] [tid: %lu]: 类装载器[%s]试图装载类型[%s]...\n", __FILE__, __LINE__, pthread_self(), _classloader->as_C_string(), class_name->as_C_string());
      JavaCalls::call_special(&result,
                              class_loader,
                              spec_klass,
...
void SystemDictionary::initialize(TRAPS) {
  // Allocate arrays
  assert(dictionary() == NULL, "SystemDictionary should only be initialized once");
...
bool SystemDictionary::initialize_wk_klass(WKID id, int init_opt, TRAPS) {
  ...
  if ((*klassp) == NULL && try_load) {
	printf("%s[%d] [tid: %lu]: 试图加载类[%s]...\n", __FILE__, __LINE__, pthread_self(), symbol->as_C_string());
 ...
 * 		2.java.lang.String
 * 		3.java.lang.Class
 * 		4.int/float/double/byte/bool/char/long/short/void
 * 		5.
 * 		6.java.lang.ref.SoftReference/java.lang.ref.WeakReference/java.lang.ref.FinalReference/java.lang.ref.PhantomReference
 */
void SystemDictionary::initialize_preloaded_classes(TRAPS) {
  assert(WK_KLASS(Object_klass) == NULL, "preloaded classes should only be initialized once");

  printf("%s[%d] [tid: %lu]: 试图加载基本的类型...\n", __FILE__, __LINE__, pthread_self());

  // Preload commonly used klasses
  WKID scan = FIRST_WKID;

  //首先加载java.lang.Object、java.lang.String、java.lang.Class三个类型
  initialize_wk_klasses_through(WK_KLASS_ENUM_NAME(Class_klass), scan, CHECK);

  //计算java.lang.String、java.lang.Class的实例属性在实例对象中的存储偏移位置

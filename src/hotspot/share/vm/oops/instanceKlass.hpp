...
class instanceKlass: public Klass {
 ...
 protected:
  ...
  typeArrayOop    _fields;	//该类型所声明的属性描述(不包括父类的属性)

  //该类型的常量池
  constantPoolOop _constants;

  // Class loader used to load this class, NULL if VM loader used.
  oop             _class_loader;	//'该类型的装载器描述对象
  ...
  int             _nonstatic_field_size;	//该类型声明的非静态属性(对象属性)占用的存储空间大小
  // number words used by static fields (oop and non-oop) in this klass
  int             _static_field_size;    //该类型声明的静态属性(类型属性)占用的存储空间大小

  u2              _static_oop_field_count;// 该类型声明的静态属性(类型属性)的数量
  u2              _java_fields_count;    // 该类型声明的属性(类型属性)的数量
  int             _nonstatic_oop_map_size;// size in words of nonstatic oop map blocks

  bool            _is_marked_dependent;  // used for marking during flushing and deoptimization
  bool            _rewritten;            // methods rewritten.
  bool            _has_nonstatic_fields; // 该类型是否有非静态属性(包括父类的非静态属性)

...
bool instanceKlass::link_class_impl( instanceKlassHandle this_oop, bool throw_verifyerror, TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 开始链接类[%s]...\n", __FILE__, __LINE__, pthread_self(), this_oop->name()->as_C_string());
  ...
  //该类已经被链接了(在当前线程链接其父类/接口的过程中，该类被其它线程完成了链接)
  if (this_oop->is_linked()) {
    return true;
  }
  ...
  {
    ObjectLocker ol(this_oop, THREAD);//上锁，阻止其它线程链接该类
...
instanceOop instanceKlass::allocate_instance(TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 试图为%s的实例分配存储空间...\n", __FILE__, __LINE__, pthread_self(), h_k->name()->as_C_string());
  i = (instanceOop)CollectedHeap::obj_allocate(h_k, size, CHECK_NULL);

  //该类型覆盖了java.lang.Object.finalize()方法
...
methodOop instanceKlass::class_initializer() {
  methodOop clinit = find_method(vmSymbols::class_initializer_name(), vmSymbols::void_method_signature());
...
void instanceKlass::call_class_initializer_impl(instanceKlassHandle this_oop, TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化类型[%s](调用它的<clinit>方法)..\n", __FILE__, __LINE__, pthread_self(), this_oop->name()->as_C_string());
...
/**
 * 根据方法名和签名从本类型声明的方法中查找搜索(不包括父类)中搜索匹配的方法
 */
...
/**
 * 根据方法名和签名从该类及其父类中递归查找该方法的描述
 * class->supper class->...->Object
 */
...
/**
 * 根据方法名和签名从该类实现的所有接口(自该类向上的整棵继承树)中查找该方法的描述
 */
methodOop instanceKlass::lookup_method_in_all_interfaces(Symbol* name, Symbol* signature) const {

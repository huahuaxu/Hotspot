...
class VM_GC_Operation: public VM_Operation {
 protected:
  ...
  unsigned int  _gc_count_before;         //本次Gc之前的数次
  unsigned int  _full_gc_count_before;    // full gc count before acquiring PLL
  bool          _full;                    // whether a "full" collection
  bool          _prologue_succeeded;      // Gc操作是否最后被执行
  GCCause::Cause _gc_cause;                //引起GC的原因
...
class VM_GenCollectForAllocation: public VM_GC_Operation {
 private:
  HeapWord*   _res;		//Gc之后分配的内存块
  size_t      _size;                       //需要在Gc完成之后分配的内存块大小
  bool        _tlab;                       //是否是申请一个线程本地分配缓冲区
 public:

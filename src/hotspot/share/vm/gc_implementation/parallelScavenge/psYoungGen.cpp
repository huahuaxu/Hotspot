...
void PSYoungGen::initialize_virtual_space(ReservedSpace rs, size_t alignment) {
  ...
  printf("%s[%d] [tid: %lu]: 试图从内存堆中为新生代申请%lu bytes的内存...\n", __FILE__, __LINE__, pthread_self(), _init_gen_size);
...
void PSYoungGen::initialize_work() {
  ...
  if (UseNUMA) {
	printf("%s[%d] [tid: %lu]: 开始创建新生代的Eden区(MutableNUMASpace)...\n", __FILE__, __LINE__, pthread_self());
    _eden_space = new MutableNUMASpace(virtual_space()->alignment());
  } else {
	printf("%s[%d] [tid: %lu]: 开始创建新生代的Eden区(MutableSpace)...\n", __FILE__, __LINE__, pthread_self());
    _eden_space = new MutableSpace(virtual_space()->alignment());
  }

  printf("%s[%d] [tid: %lu]: 开始创建新生代的From区(MutableSpace)...\n", __FILE__, __LINE__, pthread_self());
  _from_space = new MutableSpace(virtual_space()->alignment());
  printf("%s[%d] [tid: %lu]: 开始创建新生代的To区(MutableSpace)...\n", __FILE__, __LINE__, pthread_self());
  ...
  if (_eden_mark_sweep == NULL || _from_mark_sweep == NULL || _to_mark_sweep == NULL) {

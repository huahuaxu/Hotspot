...
class ParScanThreadState {
 ...
 private:
  ObjToScanQueue *_work_queue;	//待扫描复制其属性的对象队列
  ...
  HeapWord *_young_old_boundary;	//年青代与旧生代的内存边界
 ...
 public:
   ...
   * 分配指定大小的内存块
   *   分配策略:
   *          1.从当前Gc线程的内存Buffer中分配
   *          2.如果待分配的内存块大小<允许的内存缓存块碎片大小,重新给当前Gc线程分配一个内存Buffer
   *          3.直接从年青代的To区分配
   */
  HeapWord* alloc_in_to_space(size_t word_sz) {
    ...
    //申请新的一块内存Buffer来分配或直接从年青代的To区分配
...
class ParNewGenTask: public AbstractGangTask {
 private:
  ...
  Generation*                  _next_gen;   //下一个内存代(旧生代)管理器
  HeapWord*                    _young_old_boundary;		//年青代与旧生代的内存边界

  class ParScanThreadStateSet* _state_set;  //执行当前Gc任务的线程状态集

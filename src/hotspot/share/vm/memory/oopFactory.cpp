...
objArrayOop oopFactory::new_system_objArray(int length, TRAPS) {
  ...
  objArrayOop o = (objArrayOop) Universe::heap()->permanent_array_allocate(klass, size, length, CHECK_NULL);
...
methodOop oopFactory::new_method(int byte_code_size, AccessFlags access_flags,
  ...
  assert(!access_flags.is_native() || byte_code_size == 0, "native methods should not contain byte codes");

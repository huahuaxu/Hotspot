...
class Space: public CHeapObj {
 ...
 protected:
  HeapWord* _bottom;	//堆底(该内存区的物理起始地址)
  HeapWord* _end;       //堆尾(该内存区的物理结束地址)
...
private:
  HeapWord* _compaction_top;					//内存区压缩后的内存分配起始位置
  CompactibleSpace* _next_compaction_space;		//下一个可压缩的内存区
...
public:
  ...
  /**
   * 当前内存区允许压缩时的碎片率(通过允许少量的内存碎片以减少对象的移动)
   */
...
protected:
  ...
  HeapWord* _first_dead;		//内存区的压缩过程中第一个垃圾对象
  HeapWord* _end_of_live;       //内存区的压缩过程中最后一个active对象
...
class ContiguousSpace: public CompactibleSpace {
  ...
  friend class VMStructs;
 protected:
  HeapWord* _top;	//堆顶(该内存区分配内存的开始位置)
...
class EdenSpace : public ContiguousSpace {
 ...
 private:
  ...
  HeapWord* _soft_end;	//Eden区当前的逻辑(软)边界

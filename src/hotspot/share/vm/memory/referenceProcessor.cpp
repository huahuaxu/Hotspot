...
ReferencePolicy* ReferenceProcessor::_always_clear_soft_ref_policy = NULL;		//常用的软引用清理策略
ReferencePolicy* ReferenceProcessor::_default_soft_ref_policy      = NULL;		//默认的软引用清理策略
...
void ReferenceProcessor::init_statics() {
  ...
  //初始化java.lang.ref.SoftReference的静态属性clock
  ...
  guarantee(RefDiscoveryPolicy == ReferenceBasedDiscovery ||
            RefDiscoveryPolicy == ReferentBasedDiscovery, "Unrecongnized RefDiscoveryPolicy");
...
void ReferenceProcessor::process_discovered_references(
  ...
  //禁止向4个引用对类中添加新发现的Referenced类型对象
  disable_discovery();

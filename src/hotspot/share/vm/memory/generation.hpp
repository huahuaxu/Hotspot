 ...
 * 内存代管理器的抽象接口
 * 1.提供三个内存块分配方法
 * 		1).par_allocate (并行分配)多线程安全的分配方式
 * 		2).allocate 	(串行分配)调用线程使用全局锁来确保多线程安全
 * 		3).expand_and_allocate (并行+串行分配) 先扩展内存代再分配方式,由参数指定是并行还是串行分配
 */
class Generation: public CHeapObj {
 ...
 protected:
  ...
  VirtualSpace _virtual_space;	//当前内存代的内存空间
 ...
 public:
  ...
  /**
   * 是否应该在当前的内存代分配指定大小的内存块
   * 一般不建议在年青代分配大对象的内存块，以避免频繁的Gc操作
   */
  ...
  /**
   * 从当前内存代申请指定大小的内存块(该方法的内存实现可以不支持多线程安全调用)
   */
  ...
  /**
   * 来自前一个内存代的通知: 其内存分配位置到达了软限制,并决定新的分配限制位置
   * (实现可以不必考虑多线程调用的安全性,由调用者自己加锁来保证多线程安全)
   */
  ...
  /**
   * 将给定的对象(在年青代中)复制到当前内存代(年老代),该操作一般发生在年青代的Gc过程中
   *
   * 不保证多线程安全,由调用者自己使用全局锁保证线程安全
   */

...
/**
 * 将指定对象从前一个内存代升级到当前内存代
 */
oop Generation::promote(oop obj, size_t obj_size) {
  assert(obj_size == (size_t)obj->size(), "bad obj_size passed in");

#ifndef PRODUCT
  if (Universe::heap()->promotion_should_fail()) {	//对象升级当前被禁止了
    return NULL;
  }
...
void OneContigSpaceCardGeneration::collect(bool   full,
  ...
  ReferenceProcessorSpanMutator x(ref_processor(), GenCollectedHeap::heap()->reserved_region());

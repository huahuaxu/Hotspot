...
private:
  HeapWord* _start;                              // 缓冲区的开始地址
  HeapWord* _top;                                // 缓冲区分配内存的开始地址
  HeapWord* _pf_top;                             // allocation prefetch watermark
  HeapWord* _end;                                // allocation end (excluding alignment_reserve)
  size_t    _desired_size;                       // 缓冲区的基准大小   (including alignment_reserve)
  size_t    _refill_waste_limit;                 // 缓冲区重分配时允许的碎片大小

  static unsigned _target_refills;               // GC间隔中期望发生的缓冲区重分配次数

  unsigned  _number_of_refills;		//重分配本地分配缓冲区的次数

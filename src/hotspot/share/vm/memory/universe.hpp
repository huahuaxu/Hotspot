...
class Universe: AllStatic {
 ...
 public:
  ...
  enum NARROW_OOP_MODE {
    UnscaledNarrowOop  = 0,	//当整个GC堆所预留的虚拟地址范围的最高的地址在4GB以下的时候,使用"zero based Compressed Oops, 32-bits Oops"模式,
    						//也就是基地址为0、shift也为0

    ZeroBasedNarrowOop = 1,	//当GC堆的最高地址超过了4GB,但在32GB以下的时候,使用"zero based Compressed Oops"模式,
    						//也就是基地址为0、shift为 LogMinObjAlignmentInBytes(默认为3)的模式

    HeapBasedNarrowOop = 2	//当GC堆的最高地址超过了32GB,但整个GC堆的大小仍然在32GB以下的时候,

...
/**
 * 默认年青代内存管理器(由Eden/From/To三个内存区组成)
 */
class DefNewGeneration: public Generation {
  friend class VMStructs;

protected:
  Generation* _next_gen;			 //下一个内存代
  int         _tenuring_threshold;   //对象可进入下一个内存代的存活时间阈值
  ageTable    _age_table;
  ...
  Stack<oop> _promo_failure_scan_stack;	//用于存储一次Gc过程中升级(转存到下一个内存代)失败的对象
  //处理内存代垃圾回收过程中升级失败的对象
 ...
 protected:
  // Spaces
  EdenSpace*       _eden_space;		//Eden区
  ContiguousSpace* _from_space;		//From区
  ContiguousSpace* _to_space;		//To区

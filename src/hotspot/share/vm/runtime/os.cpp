...
#ifndef PRODUCT
julong os::num_mallocs = 0;         //向操作系统申请内存空间的总次数
julong os::alloc_bytes = 0;         //向操作系统申请内存空间的总大小
julong os::num_frees = 0;           //向操作系统释放申请的内存空间的总次数大小
#endif
...
static void signal_thread_entry(JavaThread* thread, TRAPS) {
  ...
  printf("%s[%d] [tid: %lu]: 线程[Signal Dispatcher]开始正是工作(核心功能)...\n", __FILE__, __LINE__, pthread_self());
  ...
  while (true) {
    ...
    if (sig == os::sigexitnum_pd()) {
       ...
       printf("%s[%d] [tid: %lu]: 收到了一个终止线程[Signal Dispatcher]的信号.\n", __FILE__, __LINE__, pthread_self());
       return;
    }

    printf("%s[%d] [tid: %lu]: 收到了一个信号[%d].\n", __FILE__, __LINE__, pthread_self(), sig);
    switch (sig) {
      case SIGBREAK: {
      ...
      default: {
        ...
        if (klass.not_null()) {
          ...
          args.push_int(sig);
          printf("%s[%d] [tid: %lu]: 调用Java中的静态方法[sun.misc.Signal.%s]来分发处理信号[%d].\n", __FILE__, __LINE__, pthread_self(), vmSymbols::dispatch_name()->as_C_string(), sig);
...
void os::signal_init() {
  if (!ReduceSignalUsage) {
    ...
    printf("%s[%d] [tid: %lu]: 开始创建[Signal Dispatcher]线程...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * 先判断当前的指针是否有足够的连续空间，如果有，扩大memblock指向的地址，并且将memblock返回;
 * 如果空间不够，先按照size指定的大小分配空间，将原有数据从头到尾拷贝到新分配的内存区域，
 * 而后释放原来memblock所指内存区域，同时返回新分配的内存区域的首地址。即重新分配存储器块的地址。
 */
...
bool os::set_boot_path(char fileSep, char pathSep) {
	...
	printf("%s[%d] [tid: %lu]: 开始初始化JVM系统类的搜索路径...\n", __FILE__, __LINE__, pthread_self());
	...
	printf("%s[%d] [tid: %lu]: 设置JVM系统类的搜索路径: (%s)...\n", __FILE__, __LINE__, pthread_self(), sysclasspath);

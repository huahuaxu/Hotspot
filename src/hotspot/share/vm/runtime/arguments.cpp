...
/**
 * 从JVM的配置文件中解析出与launcher相关的配置参数：
 * 	1). sun.java.launcher
 * 	2). sun.java.launcher.pid
 */
void Arguments::process_sun_java_launcher_properties(JavaVMInitArgs* args) {
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    if (match_option(option, "-Dsun.java.launcher.pid=", &tail)) {
      ...
      PropertyList_add(&_system_properties, new SystemProperty("sun.java.launcher.pid", tail,  false));
...
/**
 * 初始化JVM的系统属性(key-value对)(相关搜索路径)
 * 	1). java.vm.specification.name(不能修改其值)
 * 	2). java.vm.version(不能修改其值)
 * 	3). java.vm.vname(不能修改其值)
 * 	4). java.vm.info
 * 	5). java.ext.dirs
 * 	6). java.endorsed.dirs
 * 	7). sun.boot.library.path
 * 	8). java.library.path
 * 	9). java.home
 * 	10).sun.boot.class.path
 * 	11).java.class.path
 */
void Arguments::init_system_properties() {

  printf("%s[%d] [%lu]: 开始初始化JVM的系统属性...\n", __FILE__, __LINE__, pthread_self());
  ...
  //扩展类加载器(ExtClassLoader)搜索的类路径
  _java_ext_dirs = new SystemProperty("java.ext.dirs", NULL,  true);

  _java_endorsed_dirs = new SystemProperty("java.endorsed.dirs", NULL,  true);

  //JVM所依赖的(Sun实现)动态链接库搜索路径
  _sun_boot_library_path = new SystemProperty("sun.boot.library.path", NULL,  true);

  //用户java程序所依赖的第三方动态链接库搜索路径
...
/**
 * 初始化JVM的系统属性(key-value对)(与JDK版本信息相关)
 * 	1). java.vm.specification.vendor(不能修改其值)
 * 	2). java.vm.specification.version(不能修改其值)
 * 	3). java.vm.vname(不能修改其值)
 */
...
jint Arguments::parse(const JavaVMInitArgs* args) {
  ...
  printf("%s[%d] [tid: %lu]: 开始解析JVM的配置参数...\n", __FILE__, __LINE__, pthread_self());

  os::jvm_path(jvm_path, sizeof(jvm_path));
  char *end = strrchr(jvm_path, *os::file_separator());
  if (end != NULL) *end = '\0';
  char *shared_archive_path = NEW_C_HEAP_ARRAY(char, strlen(jvm_path) + strlen(os::file_separator()) + 20);

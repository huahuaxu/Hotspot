 ...
 * 初始化java.lang.System
 * 	1).初始化java执行环境的系统属性
 * 	2).初始化静态属性System.in,System.out,System.err
 * 	3).注册终结信号HUP、INT、TERM的处理函数
 */
static void call_initializeSystemClass(TRAPS) {
  klassOop k =  SystemDictionary::resolve_or_fail(vmSymbols::java_lang_System(), true, CHECK);
  instanceKlassHandle klass (THREAD, k);

  printf("%s[%d] [tid: %lu]: 开始初始化java.lang.System的静态属性...\n", __FILE__, __LINE__, pthread_self());
...
WatcherThread::WatcherThread() : Thread() {
  ...
  printf("%s[%d] [tid: %lu]: 开始创建[VM Periodic Task Thread]线程...\n", __FILE__, __LINE__, pthread_self());
  ...
  while(!_should_terminate) {
    ...
    //等待一定时间，已执行下一个最近的定时任务
    {
      ...
      for (;;) {
        ...
        if (res == OS_TIMEOUT || _should_terminate) break;
...
#endif // !SERIALGC
{
  ...
  //绑定到OS级别线程
  printf("%s[%d] [tid: %lu]: 开始为一个JVM级线程(JavaThread)创建对应的操作系统级线程并进行绑定..\n", __FILE__, __LINE__, pthread_self());
...
int JavaThread::java_suspend_self() {
  int ret = 0;
  printf("%s[%d] [tid: %lu]: java_suspend_self!\n", __FILE__, __LINE__, pthread_self());
...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
  ...
  // Parse arguments
  printf("%s[%d] [tid: %lu]: 试图解析JVM的配置参数...\n", __FILE__, __LINE__, pthread_self());
  ...
  //初始化JVM配置的所有JVMTI、JVMPI实现(查找JVM_OnLoad、Agent_OnLoad函数指针)
  if (Arguments::init_libraries_at_startup()) {
    convert_vm_init_libraries_to_agents();
  }

  //启动JVM配置的所有JVMTI实现(Agent_OnLoad函数)
  ...
  if (VerifyBeforeGC && Universe::heap()->total_collections() >= VerifyGCStartAt) {
    Universe::heap()->prepare_for_verify();
    Universe::verify();   // make sure we're starting with a clean slate
  }

  // Create the VMThread
  printf("%s[%d] [tid: %lu]: 试图创建[VM Thread]线程...\n", __FILE__, __LINE__, pthread_self());
...

  {
    ...
    //启动JVM配置的所有JVMPI实现(调用JVM_OnLoad函数)
  ...
  printf("%s[%d] [tid: %lu]: 试图创建[Signal Dispatcher]线程...\n", __FILE__, __LINE__, pthread_self());
  os::signal_init();

  // Start Attach Listener if +StartAttachListener or it can't be started lazily
  if (!DisableAttachMechanism) {
    if (StartAttachListener || AttachListener::init_at_startup()) {
      printf("%s[%d] [tid: %lu]: 试图创建[Attach Listener]线程...\n", __FILE__, __LINE__, pthread_self());
      AttachListener::init();
    }
  }
  ...
  printf("%s[%d] [tid: %lu]: 试图创建[CompilerThread*]线程...\n", __FILE__, __LINE__, pthread_self());
  ...
  if (PeriodicTask::num_tasks() > 0) {
	printf("%s[%d] [tid: %lu]: 试图创建[VM Periodic Task Thread]线程...\n", __FILE__, __LINE__, pthread_self());
    WatcherThread::start();
  }
...
/**
 * 加载指定的JVMPI、JVMTI实现(动态链接库)，并查找对应的函数指针
 */
static OnLoadEntry_t lookup_on_load(AgentLibrary* agent, const char *on_load_symbols[], size_t num_symbol_entries) {
  ...
  if (library == NULL) {
    ...
    if (agent->is_absolute_path()) {
      ...
      printf("%s[%d] [tid: %lu]: 试图加载本地链接库[%s]..\n", __FILE__, __LINE__, pthread_self(), buffer);
...
/**
 * 从指定的动态俩链接库中查找函数Agent_OnLoad的指针
 */
...
/**
 * 初始化所有的JVMTI、JVMPI实现：
 * 	加载对应的动态链接库并查找方法JVM_OnLoad、Agent_OnLoad
 */
void Threads::convert_vm_init_libraries_to_agents() {
  ...
  for (agent = Arguments::libraries(); agent != NULL; agent = next) {
    ...
    //优先从制定的JVMPI实现的动态连接库中查找函数JVM_OnLoad的指针
    OnLoadEntry_t on_load_entry = lookup_jvm_on_load(agent);

    //若没有找到查找函数JVM_OnLoad的指针，则查找函数Agent_OnLoad的指针
...
void Threads::create_vm_init_agents() {
  ...
  for (agent = Arguments::agents(); agent != NULL; agent = agent->next()) {
    ...
    if (on_load_entry != NULL) {
      printf("%s[%d] [tid: %lu]: 开始调用执行动态链接库[%s]的方法[Agent_OnLoad]...\n", __FILE__, __LINE__, pthread_self(), agent->name());
...
void Threads::shutdown_vm_agents() {
  ...
  for(AgentLibrary* agent = Arguments::agents(); agent != NULL; agent = agent->next()) {
    ...
    for(uint symbol_index = 0; symbol_index < ARRAY_SIZE(on_unload_symbols); symbol_index++) {
      printf("%s[%d] [tid: %lu]: 开始从动态链接库[%s]中查找方法[%s]...\n", __FILE__, __LINE__, pthread_self(), agent->name(), on_unload_symbols[symbol_index]);
      Agent_OnUnload_t unload_entry = CAST_TO_FN_PTR(Agent_OnUnload_t, os::dll_lookup(agent->os_lib(), on_unload_symbols[symbol_index]));
      ...
      if (unload_entry != NULL) {
        ...
        printf("%s[%d] [tid: %lu]: 开始调用执行动态链接库[%s]的方法[%s]...\n", __FILE__, __LINE__, pthread_self(), agent->name(), on_unload_symbols[symbol_index]);
...
void Threads::create_vm_init_libraries() {
  ...
  for (agent = Arguments::libraries(); agent != NULL; agent = agent->next()) {
    ...
    if (on_load_entry != NULL) {
      // Invoke the JVM_OnLoad function[JVM_OnLoad]...\n", __FILE__, __LINE__, pthread_self(), agent->name());
      JavaThread* thread = JavaThread::current();
      ThreadToNativeFromVM ttn(thread);
      HandleMark hm(thread);
      jint err = (*on_load_entry)(&main_vm, agent->options(), NULL);
...
void JavaThread::invoke_shutdown_hooks() {
  ...
  printf("%s[%d] [tid: %lu]: 开始执行用户在java程序中注册的钩子(hook)方法(在JVM停止之前).\n", __FILE__, __LINE__, pthread_self());
  ...
  EXCEPTION_MARK;
  klassOop k =
    SystemDictionary::resolve_or_null(vmSymbols::java_lang_Shutdown(), THREAD);
  if (k != NULL) {
...
bool Threads::destroy_vm() {
  ...
  printf("%s[%d] [tid: %lu]: 开始销毁JVM...\n", __FILE__, __LINE__, pthread_self());

  // Wait until we are the last non-daemon thread to execute
  { MutexLocker nu(Threads_lock);

    printf("%s[%d] [tid: %lu]: 等待所有的非守护(daemon)线程执行结束..\n", __FILE__, __LINE__, pthread_self());
    while (Threads::number_of_non_daemon_threads() > 1 )
     ...
     printf("%s[%d] [tid: %lu]: 所有的非守护(daemon)线程已执行结束.\n", __FILE__, __LINE__, pthread_self());
  }
...
void Threads::possibly_parallel_oops_do(OopClosure* f, CodeBlobClosure* cf) {
  ...
  assert(!is_par || (SharedHeap::heap()->n_par_threads() == SharedHeap::heap()->workers()->active_workers()), "Mismatch");

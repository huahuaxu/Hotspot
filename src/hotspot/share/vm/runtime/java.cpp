...
void before_exit(JavaThread * thread) {
 ...
 // printf("%s[%d] [tid: %lu]: 开始关闭JVM的所有组件..\n", __FILE__, __LINE__, pthread_self());
...
void vm_exit(int code) {
  Thread* thread = ThreadLocalStorage::is_initialized() ? ThreadLocalStorage::get_thread_slow() : NULL;
...
/**
 * 试图从动态链接库libjava.so中获取当前JVM配置的JDK版本信息
 */
void JDK_Version::initialize() {
  jdk_version_info info;
  assert(!_current.is_valid(), "Don't initialize twice");

  printf("%s[%d] [tid: %lu]: 开始初始化当前JDK的版本信息..\n", __FILE__, __LINE__, pthread_self());

...
void ServiceThread::initialize() {
  ...
  const char* name = JDK_Version::is_gte_jdk17x_version() ? "Service Thread" : "Low Memory Detector";
...

  {
    ...
    printf("%s[%d] [tid: %lu]: 开始创建[Service Thread]线程...\n", __FILE__, __LINE__, pthread_self());
...
void ServiceThread::service_thread_entry(JavaThread* jt, TRAPS) {
  printf("%s[%d] [tid: %lu]: 线程[Service Thread]开始正式工作(核心功能)...\n", __FILE__, __LINE__, pthread_self());

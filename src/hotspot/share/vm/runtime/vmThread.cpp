...
void VMThread::loop() {
  ...
  printf("%s[%d] [tid: %lu]: 线程[%s]开始正式工作(核心功能)...\n", __FILE__, __LINE__, pthread_self(), name());
  ...
  while(true) {
    ...
    { MutexLockerEx mu_queue(VMOperationQueue_lock, Mutex::_no_safepoint_check_flag);
      ...
      while (!should_terminate() && _cur_vm_operation == NULL) {
        ...
        bool timedout =
          VMOperationQueue_lock->wait(Mutex::_no_safepoint_check_flag, GuaranteedSafepointInterval);
...
void VMThread::execute(VM_Operation* op) {
  Thread* t = Thread::current();	//当前线程

  if (!t->is_VM_thread()) {	//非VMThread线程请求的JVM操作
    ...
    // to be queued up during a safepoint synchronization.
    {
      ...
      printf("%s[%d] [tid: %lu]: 试图向[VM Thread]线程的任务队列中添加一个JVM操作[%s: %s].\n", __FILE__, __LINE__, pthread_self(), t->name(), op->name());
      ...
      if(!ok) {
    	printf("%s[%d] [tid: %lu]: 放弃线程[%s]的JVM操作请求[%s].\n", __FILE__, __LINE__, pthread_self(), t->name(), op->name());
    ...
    //非并发(异步)操作，所以要一直等待JVM操作被执行完
    if (!concurrent) {
      // Wait for completion of request (non-concurrent)
      // Note: only a JavaThread triggers the safepoint check when locking
      MutexLocker mu(VMOperationRequest_lock);
      printf("%s[%d] [tid: %lu]: 当前线程[%s]开始等待JVM操作[%s]被执行完...\n", __FILE__, __LINE__, pthread_self(), t->name(), op->name());
    ...
    if (prev_vm_operation != NULL) {
      ...
      if (!prev_vm_operation->allow_nested_vm_operations()) {
        fatal(err_msg("Nested VM operation %s requested by operation %s", op->name(), vm_operation()->name()));
      }

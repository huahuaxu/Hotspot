...
JVM_ENTRY(jobject, JVM_InitProperties(JNIEnv *env, jobject properties))
  ...
  printf("%s[%d] [tid: %lu]: 开始初始化java.lang.System.props...\n", __FILE__, __LINE__, pthread_self());
  ...
  for (SystemProperty* p = Arguments::system_properties(); p != NULL; p = p->next()) {
    ...
    printf("%s[%d] [tid: %lu]: 向java.lang.System.props中添加属性[%s=%s].\n", __FILE__, __LINE__, pthread_self(), p->key(), p->value());
  }
  ...
  // from setting the value with a -D option, as requested.
  {
    ...
    printf("%s[%d] [tid: %lu]: 向java.lang.System.props中添加属性[sun.nio.MaxDirectMemorySize=%s].\n", __FILE__, __LINE__, pthread_self(), as_chars);
  }
...
#undef CSIZE
  ...
  #define CSIZE
    ...
    if (*compiler_name != '\0' &&
      ...
      printf("%s[%d] [tid: %lu]: 向java.lang.System.props中添加属性[sun.management.compiler=%s].\n", __FILE__, __LINE__, pthread_self(), compiler_name);
    }
  }
...
/**
 * 计算一个对象的hash值
 * 本地方法java.lang.Object.hashcode()的实现
 */
...
/**
 * 当前线程在指定的对象实例上等待若干长的时间
 * 本地方法java.lang.Object.wait()的实现
 */
...
JVM_ENTRY(jclass, JVM_DefineClass(JNIEnv *env, const char *name, jobject loader, const jbyte *buf, jsize len, jobject pd))
  ...
  printf("%s[%d] [tid: %lu]: 试图加载/解析(直接传二进制字节码)类[%s]...\n", __FILE__, __LINE__, pthread_self(), name);
...
JVM_ENTRY(jclass, JVM_DefineClassWithSource(JNIEnv *env, const char *name, jobject loader, const jbyte *buf, jsize len, jobject pd, const char *source))
  ...
  printf("%s[%d] [tid: %lu]: 类装载器试图加载/解析(直接传二进制字节码)类[%s]...\n", __FILE__, __LINE__, pthread_self(), name);
...
JVM_ENTRY(jclass, JVM_FindLoadedClass(JNIEnv *env, jobject loader, jstring name))
  ...
  printf("%s[%d] [tid: %lu]: 检查类[%s]是否已经被加载...\n", __FILE__, __LINE__, pthread_self(), str);
...
JVM_LEAF(jint, JVM_Close(jint fd))
  ...
  //%note jvm_r6
  printf("%s[%d] [%lu]: 试图关闭文件描述符[%d]...\n", __FILE__, __LINE__, pthread_self(), fd);
  return os::close(fd);
JVM_END
...
JVM_LEAF(jint, JVM_Read(jint fd, char *buf, jint nbytes))
  ...
  printf("%s[%d] [%lu]: 试图从文件中读取 %d个字节,文件描述符[%d]...\n", __FILE__, __LINE__, pthread_self(), nbytes, fd);
...
JVM_LEAF(jint, JVM_Write(jint fd, char *buf, jint nbytes))
  ...
  printf("%s[%d] [%lu]: 试图向文件中写入 %d个字节,文件描述符[%d]...\n", __FILE__, __LINE__, pthread_self(), nbytes, fd);
...
JVM_LEAF(jlong, JVM_Lseek(jint fd, jlong offset, jint whence))
  ...
  //%note jvm_r6
  printf("%s[%d] [%lu]: 试图移动文件当前的读/写指针到指定的位置[%d:%ld],文件描述符[%d]...\n", __FILE__, __LINE__, pthread_self(), whence, offset, fd);
...
JVM_LEAF(jint, JVM_SetLength(jint fd, jlong length))
  ...
  printf("%s[%d] [%lu]: 试图将文件的大小设置为%ld,文件描述符[%d]...\n", __FILE__, __LINE__, pthread_self(), length, fd);
...
/**
 * Java级线程核心工作的入口函数
 * 调用lang.java.Thread.run()方法
 */
static void thread_entry(JavaThread* thread, TRAPS) {
  HandleMark hm(THREAD);
  Handle obj(THREAD, thread->threadObj());	//从JVM级线程对象(JavaThread)中提取Java级线程对象(java.lang.Thread)
...
JVM_ENTRY(void, JVM_StartThread(JNIEnv* env, jobject jthread))
  ...
  typeArrayOop _name = java_lang_Thread::name(JNIHandles::resolve_non_null(jthread));
  if(_name != NULL)
	  printf("%s[%d] [tid: %lu]: 试图开启一个Java级别的线程[%s]...\n", __FILE__, __LINE__, pthread_self(), UNICODE::as_utf8((jchar*) _name->base(T_CHAR), _name->length()));
  ...
  // in Thread::start.
  {
    ...
    if (java_lang_Thread::thread(JNIHandles::resolve_non_null(jthread)) != NULL) {
      ...
      if(_name != NULL)
      	  printf("%s[%d] [tid: %lu]: 试图为一个Java级线程(java.lang.Thread)[%s]创建对应的JVM级线程(JavaThread)..\n", __FILE__, __LINE__, pthread_self(), UNICODE::as_utf8((jchar*) _name->base(T_CHAR), _name->length()));
  ...
  if (native_thread->osthread() == NULL) {//如果为该JVM级线程创建对应的OS级线程失败则抛出异常
...
JVM_ENTRY(void, JVM_StopThread(JNIEnv* env, jobject jthread, jobject throwable))
  ...
  typeArrayOop _name = java_lang_Thread::name(JNIHandles::resolve_non_null(jthread));
  if(_name != NULL)
	  printf("%s[%d] [tid: %lu]: 开始主动终止一个Java级别的线程[%s]...\n", __FILE__, __LINE__, pthread_self(), UNICODE::as_utf8((jchar*) _name->base(T_CHAR), _name->length()));
  ...
  else {//还没有为该Java级线程创建对应的JVM级线程或核心工作已经执行完或被中断
    // Either:
...
/**
 * 检查Java级线程当前是否还活着，即没有解除其与对应的JVM线程的绑定
 */
...
JVM_ENTRY(void, JVM_SuspendThread(JNIEnv* env, jobject jthread))
  ...
  typeArrayOop _name = java_lang_Thread::name(JNIHandles::resolve_non_null(jthread));
  if(_name != NULL)
	  printf("%s[%d] [tid: %lu]: 开始主动暂停一个Java级别的线程[%s]...\n", __FILE__, __LINE__, pthread_self(), UNICODE::as_utf8((jchar*) _name->base(T_CHAR), _name->length()));
...
JVM_ENTRY(void, JVM_SetThreadPriority(JNIEnv* env, jobject jthread, jint prio))
  ...
  oop java_thread = JNIHandles::resolve_non_null(jthread);
  //修改该线程的实例对象(java.lang.Thread)中属性priority值
...
JVM_ENTRY(void, JVM_Sleep(JNIEnv* env, jclass threadClass, jlong millis))
  ...
  if (millis == 0) {
    ...
    printf("%s[%d] [tid: %lu]: 当前线程[%s]开始休眠 %ld ms!\n", __FILE__, __LINE__, pthread_self(), thread->name(), millis);
...
JVM_ENTRY(jint, JVM_CountStackFrames(JNIEnv* env, jobject jthread))
  ...
  if (throw_illegal_thread_state) {
    THROW_MSG_0(vmSymbols::java_lang_IllegalThreadStateException(), "this thread is not suspended");
  }
  return count;
JVM_END

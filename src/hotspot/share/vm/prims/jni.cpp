...
JNI_ENTRY(jclass, jni_DefineClass(JNIEnv *env, const char *name, jobject loaderRef,
  ...
  klassOop k = SystemDictionary::resolve_from_stream(class_name, class_loader, Handle(), &st, true, CHECK_NULL);

  if (TraceClassResolution && k != NULL) {
    trace_class_resolution(k);
  }

  cls = (jclass)JNIHandles::make_local(env, Klass::cast(k)->java_mirror());
  return cls;
JNI_END
...
static void jni_invoke_static(JNIEnv *env, JavaValue* result, jobject receiver, JNICallType call_type, jmethodID method_id, JNI_ArgumentPusher *args, TRAPS) {
  ...
  printf("%s[%d] [%lu]: 试图执行静态方法[%s]...\n", __FILE__, __LINE__, pthread_self(), JNIHandles::resolve_jmethod_id(method_id)->name_and_sig_as_C_string());
...
static jmethodID get_method_id(JNIEnv *env, jclass clazz, const char *name_str,
  ...
  KlassHandle klass(THREAD, java_lang_Class::as_klassOop(JNIHandles::resolve_non_null(clazz)));

  // Make sure class is linked and initialized before handing id's out to
  // methodOops.
  Klass::cast(klass())->initialize(CHECK_NULL);

  printf("%s[%d] [tid: %lu]: 开始从类[%s]中查找方法[%s%s]的标识ID...\n", __FILE__, __LINE__, pthread_self(), klass->external_name(), name_str, sig);
  methodOop m;
  if (name == vmSymbols::object_initializer_name() || name == vmSymbols::class_initializer_name()) {
    ...
    if (m == NULL && klass->oop_is_instance()) {
      m = instanceKlass::cast(klass())->lookup_method_in_all_interfaces(name, signature);
    }
  }
...
/**
 * 根据方法名称及签名从该类型的描述信息中获取对应的方法标识(针对非静态方法)
 */
...
/**
 * 根据方法名称及签名从该类型的描述信息中获取对应的方法标识(针对静态方法)
 */
...
JNI_ENTRY(jobjectArray, jni_NewObjectArray(JNIEnv *env, jsize length, jclass elementClass, jobject initialElement))
 ...
 HOTSPOT_JNI_NEWOBJECTARRAY_ENTRY(
  ...
  printf("%s[%d] [tid: %lu]: 开始创建一个元素大小为%d的数组对象(%s)...\n", __FILE__, __LINE__, pthread_self(), length, ako->klass_part()->name()->as_C_string());
  objArrayOop result = objArrayKlass::cast(ak())->allocate(length, CHECK_NULL);
  oop initial_value = JNIHandles::resolve(initialElement);
  if (initial_value != NULL) {  // array already initialized with NULL
	printf("%s[%d] [tid: %lu]: 开始初始化数组对象(%s)的元素...\n", __FILE__, __LINE__, pthread_self(), ako->klass_part()->name()->as_C_string());
...
static bool register_native(KlassHandle k, Symbol* name, Symbol* signature, address entry, TRAPS) {
  ...
  if (method == NULL) {
    ResourceMark rm;
    stringStream st;
    st.print("Method %s name or signature does not match", methodOopDesc::name_and_sig_as_C_string(Klass::cast(k()), name, signature));
    THROW_MSG_(vmSymbols::java_lang_NoSuchMethodError(), st.as_string(), false);
  }

  if (!method->is_native()) {	//该方法不是本地方法
    // trying to register to a non-native method, see if a JVM TI agent has added prefix(es)
    method = find_prefixed_native(k, name, signature, THREAD);
    if (method == NULL) {
      ResourceMark rm;
      stringStream st;
      st.print("Method %s is not declared as native", methodOopDesc::name_and_sig_as_C_string(Klass::cast(k()), name, signature));
      THROW_MSG_(vmSymbols::java_lang_NoSuchMethodError(), st.as_string(), false);
    }
  }

  if (entry != NULL) {	//设置该本地方法对应的函数地址
  ...
  if (PrintJNIResolving) {
    ResourceMark rm(THREAD);
    tty->print_cr("[Registering JNI native method %s.%s]", Klass::cast(method->method_holder())->external_name(), method->name()->as_C_string());
  }
...
JNI_ENTRY(jint, jni_RegisterNatives(JNIEnv *env, jclass clazz,
  ...
  for (int index = 0; index < nMethods; index++) {
    ...
    printf("%s[%d] [tid: %lu]: 试图向JVM注册一个本地方法[%s.%s%s]的实现..\n", __FILE__, __LINE__, pthread_self(), Klass::cast(h_k())->external_name(), meth_name, meth_sig);

    // The class should have been loaded (we have an instance of the class
    // passed in) so the method and signature should already be in the symbol
    // table.  If they're not there, the method doesn't exist.
    //该类型已经被加载了，所以该方法的符号应该在符号表中可查找出
    TempNewSymbol  name = SymbolTable::probe(meth_name, meth_name_len);//查找该对应的符号
    TempNewSymbol  signature = SymbolTable::probe(meth_sig, (int)strlen(meth_sig));//查找该方法签名对应的符号
    ...
    if (!res) {
      printf("%s[%d] [tid: %lu]: 本地方法[%s.%s%s]的实现注册失败!\n", __FILE__, __LINE__, pthread_self(), Klass::cast(h_k())->external_name(), meth_name, meth_sig);
      ret = -1;
      break;
    }
  }
...
/**
 * 获取默认的JVM初始化参数(当前JVM实现支持的版本、方法栈大小)
 */
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_GetDefaultJavaVMInitArgs(void *args_) {
  ...
  if (args->version == JNI_VERSION_1_1) {
    ...
    printf("%s[%d] [%lu]: JVM中默认的java线程栈大小为 %d bytes.\n", __FILE__, __LINE__, pthread_self(), args->javaStackSize);
  }
...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
  ...
  if (result == JNI_OK) {
    ...
    *(JNIEnv**)penv = thread->jni_environment();
    ...
    EVENT_COMMIT(event, EVENT_SET(event, javalangthread, java_lang_Thread::thread_id(thread->threadObj())));

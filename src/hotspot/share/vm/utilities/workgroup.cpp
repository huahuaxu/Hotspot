...
void GangWorker::loop() {
  ...
  for ( ; /* !terminate() */; ) {
    ...
    int part;  // Initialized below.
    {
      ...
      for ( ; /* break or return */; ) {
        ...
        if ((data.task() != NULL) && (data.sequence_number() != previous_sequence_number)) {
          if (gang()->needs_more_workers()) {	//新任务还需工作线程执行
    ...
    if (TraceWorkGang) {
      tty->print("Work for work gang %s id %d task %s part %d", gang()->name(), id(), data.task()->name(), part);
    }
    assert(data.task() != NULL, "Got null task");

    printf("%s[%d] [tid: %lu]: 工作线程[%s:%d]开始执行任务[%s--%d]...\n", __FILE__, __LINE__, pthread_self(),

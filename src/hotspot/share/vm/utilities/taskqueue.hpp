 ...
 * 获取指定的任务队列
 */
template<class T> T* GenericTaskQueueSet<T>::queue(uint i) {
  return _queues[i];
}

template<class T> bool GenericTaskQueueSet<T>::steal(uint queue_num, int* seed, E& t) {
...
/**
 * 偷取最长队列中的一个任务.
 * 如果当前任务队列集中只有两个队列则随机偷取其中一个队列的任务;如果当前任务队列集中只有一个队列则放弃偷窃
 */

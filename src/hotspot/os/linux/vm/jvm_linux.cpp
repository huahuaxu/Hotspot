...
JVM_ENTRY_NO_ENV(void*, JVM_RegisterSignal(jint sig, void* handler))
  ...
  switch (sig) {
    ...
    case BREAK_SIGNAL:
    	printf("%s[%d] [tid: %lu]: 开始注册(重置)信号[%d]时被忽略, 因为该信号已经被JVM所使用..\n", __FILE__, __LINE__, pthread_self(), sig);
      return (void *)-1;
  ...
  printf("%s[%d] [tid: %lu]: 开始注册(重置)信号[%d]的处理函数..\n", __FILE__, __LINE__, pthread_self(), sig);
...
JVM_ENTRY_NO_ENV(jint, JVM_FindSignal(const char *name))
  ...
  for(uint i=0; i<ARRAY_SIZE(siglabels); i++){
    if(!strcmp(name, siglabels[i].name)){
    	printf("%s[%d] [tid: %lu]: 查找到信号名[%s]对应的编号[%d].\n", __FILE__, __LINE__, pthread_self(), name, siglabels[i].number);
    	return siglabels[i].number;
    }
  }

  printf("%s[%d] [tid: %lu]: 没有找到信号名[%s]对应的编号.\n", __FILE__, __LINE__, pthread_self(), name);

...
class PlatformEvent : public CHeapObj {
  ...
  public:
     ...
     * 函数说明：以动态方式创建互斥锁的，参数attr指定了新建互斥锁的属性。如果参数attr为空，
     * 		   则使用默认的互斥锁属性，默认属性为快速互斥锁。
     * 参数说明： pthread_mutexattr_t：
     * 					PTHREAD_MUTEX_NORMAL 		不提供死锁检测，尝试重新锁定互斥锁会导致死锁。
     * 								 				如果某个线程尝试解除锁定的互斥锁不是由该线程锁定或未锁定，
     * 								 				则将产生不确定的行为。
     * 					PTHREAD_MUTEX_ERRORCHECK 	提供错误检查，如果某个线程尝试重新锁定的互斥锁已经由该线程
     * 									 			锁定，则将返回错误。如果某个线程尝试解除锁定的互斥锁不是由
     * 									 			该线程锁定或者未锁定，则将返回错误。
     * 					PTHREAD_MUTEX_RECURSIVE  	互斥锁会保留锁定计数这一概念。线程首次成功获取互斥锁时，
     * 									 			锁定计数会设置为1。线程每重新锁定该互斥锁一次，锁定计数就
     * 									 			增加1。线程每解除锁定该互斥锁一次，锁定计数就减小1。锁定计
     * 									 			数达到0时，该互斥锁即可供其他线程获取。如果某个线程尝试解
     * 									 			除锁定的互斥锁不是由该线程锁定或者未锁定，则将返回错误。
     * 					PTHREAD_MUTEX_DEFAULT	 	尝试以递归方式锁定该互斥锁将产生不确定的行为。对于不是由
     * 									 			调用线程锁定的互斥锁，如果尝试解除对它的锁定，则会产生不确定
     * 									 			的行为。如果尝试解除锁定尚未锁定的互斥锁，则会产生不确定的行为。
     * 函数返回：成功完成之后会返回零，其他任何返回值都表示出现了错误。
     *
     * 函数原型：int pthread_cond_init(pthread_cond_t *cv, const pthread_condattr_t *cattr);
     * 函数说明：初始化一个条件变量
     * 参数说明：pthread_condattr_t：
     * 					PTHREAD_ PROCESS_PRIVATE	此条件变量被同一进程内的各个线程使用。(默认值)
     * 					PTHREAD_PROCESS_SHARED		多个进程间各线程公用。
     */
    PlatformEvent() {
      int status;

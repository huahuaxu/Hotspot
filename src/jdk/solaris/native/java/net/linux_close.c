 ...
 * 函数说明: 检测指定的socket描述符上有没有指定的事件发生. 如果所检测的socket描述符上没有任何事件发生的话，那么:
 *           	1.timeout==0，那么poll()函数立即返回而不阻塞;
 *           	2.poll()函数会阻塞timeout所指定的毫秒时间长度之后返回
 *           	3.timeout==INFTIM，那么poll()函数会一直阻塞下去，直到所检测的socket描述符上的感兴趣的事件发生是才返回，
 ...
 *         	};
 *
 *         nfds_t 用于标记数组fds中的结构体元素的总数量
 *         timeout 是poll函数调用阻塞的时间，单位：毫秒
 *
 * 返回值:
 *        1).>0  数组fds中准备好读、写或出错状态的那些socket描述符的总数量
 *        2).=0  数组fds中没有任何socket描述符准备好读、写，或出错,此时poll超时，超时时间是timeout毫秒
...
int NET_Timeout(int s, long timeout) {
    ...
    for(;;) {
         ...
         */
        pfd.fd = s;
        pfd.events = POLLIN | POLLERR; //检测socket是否普通或优先级带数据可读、是否发生错误

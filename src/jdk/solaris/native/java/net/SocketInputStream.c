                                            ...
                                            jint off, jint len, jint timeout)
{
    ...
    if (IS_NULL(fdObj)) { //Java文件描述符java.io.FileDescriptor对象为空
    	...
    	//从Java文件描述符java.io.FileDescriptor对象中取出内核级文件描述符fd
    ...
    if (len > MAX_BUFFER_LEN) {
        ...
        if (bufP == NULL) {	//向操作系统申请内存失败,则用方法栈中的静态空间
            bufP = BUF;
            len = MAX_BUFFER_LEN;
        }
    } else {
    ...
    if (nread <= 0) {
    	...
    	//将读取到的数据复制到指定的Java byte数组对象中
        (*env)->SetByteArrayRegion(env, data, off, nread, (jbyte *)bufP);
    }

    if (bufP != BUF) {	//释放动态申请的空间
        free(bufP);
    }

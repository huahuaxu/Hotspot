...
/*
 * 搜索zip文件的压缩源文件目录区结束偏移位置,并读取压缩源文件目录结束标志区
 */
static off_t find_end(int fd, Byte *eb)
{
    off_t   len;
    off_t   pos;
    off_t   flen;
...
int JLI_ParseManifest(char *jarfile, manifest_info *info)
{
    ...
    printf("%s[%d] [tid: %lu]: 开始从jar包[%s]中加载java主运行程序的相关信息...\n", __FILE__, __LINE__, pthread_self(), jarfile);
    ...
    printf("%s[%d] [tid: %lu]: 试图从jar包[%s]中加载文件[%s]的基本信息...\n", __FILE__, __LINE__, pthread_self(), jarfile, manifest_name);
    if (rc = find_file(fd, &entry, manifest_name) != 0) {
        close(fd);
        printf("%s[%d] [tid: %lu]: 无法从jar包[%s]中找到文件[%s].\n", __FILE__, __LINE__, pthread_self(), jarfile, manifest_name);
        return (-2);
    }

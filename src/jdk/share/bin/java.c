...
)
{
    ...
    printf("%s[%d] [tid: %lu]: 试图加载JVM的实现(动态连接库:%s)..\n", __FILE__, __LINE__, pthread_self(), jvmpath);
    if (!LoadJavaVM(jvmpath, &ifn)) {
    	printf("%s[%d] [tid: %lu]: 加载JVM[%s]失败.\n", __FILE__, __LINE__, pthread_self(), jvmpath);
        return(6);
    }
    ...
    if (mode == LM_JAR) {
    	printf("%s[%d] [tid: %lu]: java主程序在jar包中.\n", __FILE__, __LINE__, pthread_self());
...
int JNICALL JavaMain(void * _args)
{
    ...
    JavaVM *vm = 0;		//JVM实例
    JNIEnv *env = 0;	//JVM环境
    jclass mainClass = NULL;	//Java主运行程序类
    jmethodID mainID;			//main方法的标识
    jobjectArray mainArgs;		//main方法的参数
     ...
     */
    printf("%s[%d] [tid: %lu]: 试图获取java主程序[%s]的main方法入口..\n", __FILE__, __LINE__, pthread_self(), what);
    ...
    if(ret) printf("%s[%d] [tid: %lu]: java主程序[%s]的main方法以抛出异常结束.\n", __FILE__, __LINE__, pthread_self(), what);
    else printf("%s[%d] [tid: %lu]: java主程序[%s]的main方法正常结束.\n", __FILE__, __LINE__, pthread_self(), what);
...
AddOption(char *str, void *info)
{
    ...
    if (numOptions >= maxOptions) {
        if (options == 0) {		//申请内存空间(数组)来保存JVM参数
            maxOptions = 4;
            options = JLI_MemAlloc(maxOptions * sizeof(JavaVMOption));
        } else {	//扩展JVM参数的存储空间(增加到2倍)
            JavaVMOption *tmp;
            maxOptions *= 2;
    ...
    if (JLI_StrCCmp(str, "-Xss") == 0) {
        ...
        if(parse_size(str + 4, &tmp)) {
            threadStackSize = tmp;
            printf("%s[%d] [%lu]: JVM中每个线程栈大小为 %lu bytes.\n", __FILE__, __LINE__, pthread_self(), threadStackSize);
        }
    }
    ...
    if (JLI_StrCCmp(str, "-Xmx") == 0) {
        ...
        if (parse_size(str + 4, &tmp)) {
            maxHeapSize = tmp;
            printf("%s[%d] [%lu]: JVM的最大堆大小为 %lu bytes.\n", __FILE__, __LINE__, pthread_self(), maxHeapSize);
        }
    }
    ...
    if (JLI_StrCCmp(str, "-Xms") == 0) {
        ...
        if (parse_size(str + 4, &tmp)) {
           initialHeapSize = tmp;
           printf("%s[%d] [%lu]: JVM的最小(初始化)堆大小为 %lu bytes.\n", __FILE__, __LINE__, pthread_self(), initialHeapSize);
        }
    }
}
...
static void SelectVersion(int argc, char **argv, char **main_class)
{
    ...
    char    **new_argv;
    char    **new_argp;
    char    *operand;				//java的程序类或者jar文件
    char    *version = NULL;
    char    *jre = NULL;
    int     jarflag = 0;			//java主运行程序是否在jar包中
    int     headlessflag = 0;
    int     restrict_search = -1;               /* -1 implies not known */
    manifest_info info;
    char    env_entry[MAXNAMELEN + 24] = ENV_ENTRY "=";
    char    *splash_file_name = NULL;	//程序启动时的画面对应的图片文件路径
     ...
     * 函数说明:	用来取得参数envvar环境变量的内容.参数envvar为环境变量的名称,
     * 			如果该变量存在则会返回指向该内容的指针
     * 函数说明:	执行成功则返回指向该内容的指针,找不到符合的环境变量名称则返回NULL.
     * 			如果变量存在但无关联值,它将运行成功并返回一个空字符串,即该字符的第一个字节是null
     */
    if ((env_in = getenv(ENV_ENTRY)) != NULL) {	//java主程序类已经在环境变量表中设置了，则将主运行类赋值给指定的变量后直接返回
        if (*env_in != '\0')
    ...
    while ((arg = *argv) != 0 && *arg == '-') {
        if (JLI_StrCCmp(arg, "-version:") == 0) {	//JVM级参数指定了运行时的jre版本
            version = arg + 9;
        } else if (JLI_StrCmp(arg, "-jre-restrict-search") == 0) {	//在版本搜索的时候,包含用户私人的JRE
            restrict_search = 1;
        } else if (JLI_StrCmp(arg, "-no-jre-restrict-search") == 0) {	//在版本搜索的时候，排除用户私人的JRE
            restrict_search = 0;
        } else {
            if (JLI_StrCmp(arg, "-jar") == 0) jarflag = 1;
             ...
             * 检查配置JVM中图形化工具的实现方式
             */
            if (JLI_StrCmp(arg, "-Djava.awt.headless=true") == 0) {	//系统实现
                headlessflag = 1;
            } else if (JLI_StrCCmp(arg, "-Djava.awt.headless=") == 0) {	//特定于平台的默认实现
                headlessflag = 0;
            } else if (JLI_StrCCmp(arg, "-splash:") == 0) {	//程序启动时的画面
    ...
    *new_argp = NULL;	//最后一个参数标记为NULL
    ...
    if (jarflag && operand) {
        ...
        /*
         * 命令行中没有配置启动画面而主jar包中配置了
         */
        if (!headlessflag && !splash_file_name && info.splashscreen_image_file_name) {
            splash_file_name = info.splashscreen_image_file_name;
            splash_jar_name = operand;
        }
    } else {	//java主程序在指定的class文件中
    ...
    printf("%s[%d] [tid: %lu]: 开始使用指定的jre版本[%s]来执行java程序...\n", __FILE__, __LINE__, pthread_self(), info.jre_version);
 ...
 */
        	...
        	printf("%s[%d] [%lu]: 添加JVM运行时属性[%s].\n", __FILE__, __LINE__, pthread_self(), arg);
            AddOption(arg, NULL);
        }
    }
...
static jboolean InitializeJVM(JavaVM **pvm, JNIEnv **penv, InvocationFunctions *ifn)
{
    ...
    if (JLI_IsTraceLauncher()) {
        ...
        printf("ignoreUnrecognized is %s, ", args.ignoreUnrecognized ? "JNI_TRUE" : "JNI_FALSE");
...
static jclass GetLauncherHelperClass(JNIEnv *env) {
    if (helperClass == NULL) {
    	printf("%s[%d] [tid: %lu]: 试图获取/加载类[sun.launcher.LauncherHelper]...\n", __FILE__, __LINE__, pthread_self());
...
SetJavaCommandLineProp(char *what, int argc, char **argv)
{
    ...
    *javaCommand = '\0';
    ...
    printf("%s[%d] [tid: %lu]: 添加JVM运行时属性[%s].\n", __FILE__, __LINE__, pthread_self(), javaCommand);
...
void
SetJavaLauncherProp() {
  printf("%s[%d] [tid: %lu]: 添加JVM运行时属性[-Dsun.java.launcher=SUN_STANDARD].\n", __FILE__, __LINE__, pthread_self());
...
ShowSettings(JNIEnv *env, char *optString)
{
    ...
    jclass cls = GetLauncherHelperClass(env);
    NULL_CHECK(cls);
    NULL_CHECK(showSettingsID = (*env)->GetStaticMethodID(env, cls, "showSettings", "(ZLjava/lang/String;JJJZ)V"));
                    ...
                    int mode, char *what, int ret)
{
    ...
    if (threadStackSize == 0) {
      ...
      printf("%s[%d] [%lu]: 试图获取默认的JVM初始化参数(方法栈大小)...\n", __FILE__, __LINE__, pthread_self());
    ...
    { /* Create a new thread to create JVM and invoke main method */
      ...
      printf("%s[%d] [tid: %lu]: 试图创建一个新线程来运行JVM..\n", __FILE__, __LINE__, pthread_self());
